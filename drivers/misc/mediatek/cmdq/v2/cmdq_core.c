...
static int32_t cmdq_core_check_task_valid(TaskStruct *pTask)
{
	uint64_t *va;
	bool ret = true;

	for (va = (uint64_t *)pTask->pVABase; ret && (unsigned long)va <
		(unsigned long)pTask->pVABase + pTask->bufferSize; va++)
