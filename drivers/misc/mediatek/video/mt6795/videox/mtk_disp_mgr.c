...
int _ioctl_trigger_session(unsigned long arg)
{
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
		DISPPR_FENCE("T+/%s%d/%d/%d\n", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id),
			     config.present_fence_idx, config.need_merge);
	} else {
		DISPPR_FENCE("T+/%s%d\n", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id));
	}
...
#ifdef CONFIG_MTK_HDMI_SUPPORT
		mutex_lock(&disp_session_lock);
		/*ret = ext_disp_trigger(0, NULL ,0,  session_id);*/
		ret = external_display_trigger(TRIGGER_NORMAL, session_id);
...
int _ioctl_prepare_buffer(unsigned long arg, ePREPARE_FENCE_TYPE type)
{
	...
	if (info.layer_en) {
		...
		DISPPR_ERROR("P+ FAIL /%s%d/l%d/e%d/ion%d/c%d/id%d/ffd%d\n", disp_session_mode_spy(info.session_id),
			     DISP_SESSION_DEV(info.session_id), info.layer_id, info.layer_en, info.ion_fd,
						...
						OVL_CONFIG_STRUCT *dst, unsigned int dst_mva)
{
	...
	if (DISP_FORMAT_XRGB8888 == src->src_fmt ||
	    ...
	    DISP_FORMAT_RGBX8888 == src->src_fmt || DISP_FORMAT_BGRX8888 == src->src_fmt)
		dst->aen = FALSE;
...
static int _sync_convert_fb_layer_to_disp_input(unsigned int session_id, disp_input_config *src,
						primary_disp_input_config *dst, unsigned int dst_mva)
{
	unsigned int layerpitch = 0;
	unsigned int layerbpp = 0;
	...
	for (i = 0; i < input->config_layer_num; i++) {
		...
		if (session_info) {
			dprec_submit(&session_info->event_setinput, input->config[i].next_buff_idx,
				     (input->config_layer_num << 28) | (input->config[i].layer_id << 24) | (input->
													    config[i].
													    src_fmt <<
													    12) |
				     input->config[i].layer_enable);
...
static int set_external_buffer(disp_session_input_config *input)
{
	...
	session_id = input->session_id;
	disp_session_sync_info *session_info = disp_get_session_sync_info_for_debug(session_id);
	/*if(get_ext_disp_path_mode() == EXTD_RDMA_DPI_MODE)
	...
	for (i = 0; i < input->config_layer_num; ++i) {
		...
		if (input->config[i].layer_enable) {
			...
			DISPPR_FENCE("S+/EL%d/e%d/id%d/%dx%d(%d,%d)(%d,%d)/%s/%d/0x%08lx/mva0x%08x\n",
				input->config[i].layer_id,
				input->config[i].layer_enable,
				input->config[i].next_buff_idx,
				input->config[i].src_width,
...
static int set_primary_buffer(disp_session_input_config *input)
{
	...
	for (i = 0; i < input->config_layer_num; i++) {
		...
		if (input->config[i].layer_enable) {
			...
			DISPPR_FENCE("S+/PL%d/e%d/id%d/%dx%d(%d,%d)(%d,%d)/%s/%d/0x%08lx/mva0x%08x/sec%d\n",
				     input->config[i].layer_id,
				     input->config[i].layer_enable,
				     input->config[i].next_buff_idx,
				     input->config[i].src_width,
		...
		disp_sync_put_cached_layer_info(session_id, layer_id, &input->config[i], dst_mva);
		_sync_convert_fb_layer_to_disp_input(input->session_id, &(input->config[i]), &primary_input[layer_id],
						     dst_mva);

		if (input->config[i].layer_enable) {
			/* OVL addr is not the start address of buffer, which is calculated by pitch and ROI. */
			mva_offset =
				primary_input[layer_id].src_x * (primary_input[layer_id].src_pitch /
								 input->config[i].src_pitch) +
...
int _ioctl_set_output_buffer(unsigned long arg)
{
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
		...
		_sync_convert_fb_layer_to_disp_output(session_output.session_id, &(session_output.config),
		...
		if (primary_display_is_decouple_mode()) {
			...
			disp_sync_put_cached_layer_info(session_id, disp_sync_get_output_interface_timeline_id(), &src,
							dst_mva);
		...
		mtkfb_update_buf_ticket(session_id, disp_sync_get_output_timeline_id(), session_output.config.buff_idx,
					get_ovl2mem_ticket());
		_sync_convert_fb_layer_to_disp_output(session_output.session_id, &(session_output.config),
						      &primary_output, dst_mva);
		primary_output.dirty = 1;

		DISPPR_FENCE("S+/%s%d/L%d/id%d/%dx%d(%d,%d)/%s/%d/%d/0x%08lx/mva0x%08x/t%d/sec%d(%d)\n",
			     disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id),
			     4,
			     session_output.config.buff_idx,
			     session_output.config.width,
			     ...
			     dst_mva, get_ovl2mem_ticket(), session_output.config.security, primary_output.security);
...
int _ioctl_wait_vsync(unsigned long arg)
{
	...
	if (ret == 0) {
		if (copy_to_user(argp, &vsync_config, sizeof(vsync_config)))
			DISPMSG("vsy: copy_to_user failed! line:%d\n", __LINE__);
...
long mtk_disp_mgr_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	/* DISPMSG("mtk_disp_mgr_ioctl, cmd=%s, cmd nr=0x%08x\n",
		_session_ioctl_spy(cmd), (unsigned int)_IOC_NR(cmd)); */
