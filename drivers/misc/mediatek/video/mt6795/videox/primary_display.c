...
static int __primary_display_switch_path(int sess_mode, unsigned int session, int need_lock, enum DISP_USER user);
...
typedef struct {
	...
	disp_path_handle dpmgr_sub_mem_path_handle;
	cmdqRecHandle cmdq_handle_sub_mem_config;
#endif
	...
	enum DECOUPLE_TYPE decouple_type;
	int ovl_one_layer;
	int test_mode;
	int mhl_connect;
	int delayed_fps;
...
static int primary_is_sec(void)
{
...
static void primary_display_idlemgr_enter_idle(int need_lock)
{
	...
	if (primary_display_is_video_mode() &&
		...
		if (!disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)) {
			...
			__primary_display_switch_path(DISP_SESSION_DECOUPLE_MODE, pgc->session_id, need_lock,
						      DISP_USER_IDLE);
			...
			if (ret == 0) {
				...
				if (pgc->plcm->params->lcm_if == LCM_INTERFACE_DSI_DUAL) {
					...
					cmdqRecWrite(handle, 0x1401b028,
						     pgc->plcm->params->dsi.vertical_frontporch_for_low_power, ~0);
...
static void primary_display_idlemgr_leave_idle(int need_lock)
{
	...
	if (primary_display_is_video_mode() &&
		...
		if (pgc->plcm->params->dsi.vertical_frontporch_for_low_power) {
			...
			if (ret == 0) {
				...
				if (pgc->plcm->params->lcm_if == LCM_INTERFACE_DSI_DUAL) {
					...
					cmdqRecWrite(handle, 0x1401b028, pgc->plcm->params->dsi.vertical_frontporch,
						     ~0);
					cmdqRecWrite(handle, 0x1401c028, pgc->plcm->params->dsi.vertical_frontporch,
						     ~0);
				} else if (pgc->plcm->params->lcm_if == LCM_INTERFACE_DSI0) {
		...
		if (!disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)) {
			...
			__primary_display_switch_path(session_mode_before_enter_idle, pgc->session_id, need_lock,
						      DISP_USER_IDLE);
		}
...
int primary_display_get_debug_state(char *stringbuf, int buf_len)
{
	...
	if (lcm_param && lcm_drv)
		len += scnprintf(stringbuf + len, buf_len - len,
			...
			lcm_param->width, lcm_param->height, (lcm_param->type == LCM_TYPE_DSI) ? "DSI" : "Other");
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (primary_display_is_video_mode()) {
		...
		if (pgc->decouple_type == DECOUPLE_UFOE)
		...
		if (primary_display_get_lcm_max_refresh_rate() != 120) {
			...
			/* Enable SPM CG Mode(Force 30+ times to ensure write success,
			need find root cause and fix later) */
...
static void update_frm_seq_info(unsigned int addr, unsigned int addr_offset, unsigned int seq, DISP_FRM_SEQ_STATE state)
{
	...
	if (FRM_CONFIG == state) {
		...
		MMProfileLogEx(ddp_mmp_get_events()->primary_seq_trigger, MMProfileFlagPulse, frm_update_cnt,
...
static void directlink_path_add_memory(WDMA_CONFIG_STRUCT *p_wdma, ENGINE_DUMP after_engine)
{
	int ret = 0;
	...
	if (pgc->decouple_type == DECOUPLE_UFOE) {
		pconfig->wdma_config.srcWidth /= 3;
		pconfig->wdma_config.clipWidth /= 3;
		pconfig->wdma_config.dstPitch /= 3;
	}
	pconfig->wdma_dirty = 1;
	ret = dpmgr_path_config(pgc->dpmgr_handle, pconfig, cmdq_handle);
	/* clear sof for next frame */
...
static int _DL_switch_to_DC_fast_two_pipe(void)
{
	...
	directlink_path_add_memory(&wdma_config, ENGINE_OD);
	...
	dpmgr_path_trigger(pgc->dpmgr_handle_two_pipe, pgc->cmdq_handle_config,
...
static int _DL_switch_to_DC_fast(void)
{
	...
	/* 1.save a temp frame to intermediate buffer */
	if (pgc->decouple_type == DECOUPLE_UFOE)
		directlink_path_add_memory(&wdma_config, ENGINE_UFOE);
	else
		directlink_path_add_memory(&wdma_config, ENGINE_OVL0);
	...
	if (decouple_shorter_path)
		new_scenario = DDP_SCENARIO_PRIMARY_RDMA0_DISP;
	else if (pgc->decouple_type == DECOUPLE_UFOE)
		new_scenario = DDP_SCENARIO_SUB_RDMA1_DISP;
	else if (pgc->decouple_type == DECOUPLE_OD)
		new_scenario = DDP_SCENARIO_PRIMARY_RDMA0_DISP;
	else
	...
	data_config_dl->rdma_config = rdma_config;
	if (pgc->decouple_type == DECOUPLE_UFOE) {
		data_config_dl->rdma_config.width /= 3;
		data_config_dl->rdma_config.pitch /= 3;
	}
	data_config_dl->rdma_dirty = 1;
	ret = dpmgr_path_config(pgc->dpmgr_handle, data_config_dl, pgc->cmdq_handle_config);
	...
	if (pgc->decouple_type == DECOUPLE_UFOE) {
		pgc->ovl2mem_path_handle =
		    dpmgr_create_path(DDP_SCENARIO_PRIMARY_UFOE_MEMOUT, pgc->cmdq_handle_ovl1to2_config);
	} else if (pgc->decouple_type == DECOUPLE_OD) {
		pgc->ovl2mem_path_handle =
		    dpmgr_create_path(DDP_SCENARIO_PRIMARY_OD_MEMOUT, pgc->cmdq_handle_ovl1to2_config);
	} else {
		pgc->ovl2mem_path_handle =
	...
	if (pgc->decouple_type == DECOUPLE_UFOE) {
		/* ufoe need lcm info , so copy config info from dl to dc */
		memcpy(data_config_dc, data_config_dl, sizeof(disp_ddp_path_config));
	} else {
		/* copy  ovl info from dl to dc */
		memcpy(data_config_dc->ovl_config, data_config_dl->ovl_config, sizeof(data_config_dl->ovl_config));
	}
	data_config_dc->rdma_config.address = 0;
	data_config_dc->dst_w = rdma_config.width;
	data_config_dc->dst_h = rdma_config.height;
	data_config_dc->dst_dirty = 1;
	data_config_dc->rdma_dirty = 0;
	data_config_dc->ovl_dirty = 0;
...
static int _DL_switch_to_DC_fast_sw_only(void)
{
...
#if CONFIG_FOR_SOURCE_PQ
	pgc->ovl2mem_path_handle =
		dpmgr_create_path(DDP_SCENARIO_PRIMARY_DITHER_MEMOUT, pgc->cmdq_handle_ovl1to2_config);
#else
	if (pgc->decouple_type == DECOUPLE_UFOE)
		pgc->ovl2mem_path_handle = dpmgr_create_path(DDP_SCENARIO_PRIMARY_UFOE_MEMOUT,
				pgc->cmdq_handle_ovl1to2_config);
	else if (pgc->decouple_type == DECOUPLE_OD)
		pgc->ovl2mem_path_handle = dpmgr_create_path(DDP_SCENARIO_PRIMARY_OD_MEMOUT,
				pgc->cmdq_handle_ovl1to2_config);
	else
	...
	dpmgr_enable_event(pgc->dpmgr_handle, DISP_PATH_EVENT_FRAME_START);
	if (!primary_display_is_video_mode()) {
		/* ovl is reset in trigger loop when command mode.
		 ** trigger loop need to be rebuild.
		 */
		_cmdq_build_trigger_loop();
	}
	/* wdma is idle ,so set token */
...
static int modify_path_power_off_callback(uint32_t userdata)
{
	DDP_SCENARIO_ENUM old_scenario, new_scenario;
...
static int _DC_switch_to_DL_fast(void)
{
	...
	/* to make sure all wdma task is done! */
	_cmdq_flush_config_handle_mira(pgc->cmdq_handle_ovl1to2_config, 1);
	...
	if (!primary_display_is_video_mode()) {
		/* update decouple, rebuild decouple mode */
		pgc->decouple_type = DECOUPLE_OVL;
...
#ifdef CONFIG_FOR_SOURCE_PQ
void primary_backup_mira_config(void)
{
	DISP_REG_SET(NULL, DISP_REG_CONFIG_DISP_WDMA1_SEL_IN, 0);

	DISP_REG_SET_FIELD(NULL, REG_FLD(1, 0), DISP_REG_CONFIG_DISP_OVL1_MOUT_EN, 0);
	DISP_REG_SET_FIELD(NULL, REG_FLD(1, 2), DISP_REG_CONFIG_DISP_GAMMA_MOUT_EN, 0);
}

void primary_restore_mira_config(void)
{
	DISP_REG_SET(NULL, DISP_REG_CONFIG_DISP_WDMA1_SEL_IN, 1);

	DISP_REG_SET_FIELD(NULL, REG_FLD(1, 0), DISP_REG_CONFIG_DISP_OVL1_MOUT_EN, 1);
	DISP_REG_SET_FIELD(NULL, REG_FLD(1, 2), DISP_REG_CONFIG_DISP_GAMMA_MOUT_EN, 1);
}

int _build_path_memout_mira(void)
{
	int ret = 0;
	DISP_MODULE_ENUM dst_module = 0;
	DISPFUNC();

	pgc->dpmgr_sub_mem_path_handle =
		dpmgr_create_path(DDP_SCENARIO_SUB_GAMMA_MEMOUT, pgc->cmdq_handle_sub_mem_config);
	if (pgc->dpmgr_sub_mem_path_handle) {
		DISPCHECK("dpmgr create path SUCCESS(%p)\n", pgc->dpmgr_sub_mem_path_handle);
	} else {
		DISPCHECK("dpmgr create path FAIL\n");
		return -1;
	}
	dpmgr_path_init(pgc->dpmgr_sub_mem_path_handle, disp_helper_get_option(DISP_HELPER_OPTION_USE_CMDQ));
	dpmgr_path_start(pgc->dpmgr_sub_mem_path_handle, disp_helper_get_option(DISP_HELPER_OPTION_USE_CMDQ));
#ifndef MTKFB_NO_M4U
	M4U_PORT_STRUCT sPort;
...
int _trigger_ovl_to_memory(disp_path_handle disp_handle,
			   cmdqRecHandle cmdq_handle, fence_release_callback callback, unsigned int data)
{
		...
		if (ret == 1) {
			...
			while (i--) {
				...
				if (ret == 0) {
					...
					if (i == 0) {
						DISPCHECK
						    ("[ESD]after esd recovery %d times, check still fail, disable\n",
						     esd_try_cnt);
...
int primary_display_change_lcm_resolution(unsigned int width, unsigned int height)
{
	if (pgc->plcm) {
		...
		if (width > pgc->plcm->params->width || height > pgc->plcm->params->height || width == 0 || height == 0
		    || width % 4 || height % 4) {
...
static int _decouple_update_rdma_config(void)
{
	int ret;
	_primary_path_lock(__func__);
...
static int _ovl_fence_release_callback(uint32_t userdata)
{
	...
	for (i = 0; i < PRIMARY_DISPLAY_SESSION_LAYER_COUNT; i++) {
		...
		MMProfileLogEx(ddp_mmp_get_events()->primary_ovl_fence_release, MMProfileFlagPulse, i,
...
static int update_rdma_config_thread(void *data);
...
static int _present_fence_release_worker_thread(void *data)
{
	...
	while (1) {
		...
		MMProfileLogEx(ddp_mmp_get_events()->present_fence_release, MMProfileFlagPulse, gPresentFenceIndex,
			       fence_increment);
		_primary_path_unlock(__func__);
	}
...
static int update_primary_intferface_module(void)
{
	...
	ddp_set_dst_module(DDP_SCENARIO_PRIMARY_OD_1TO2, interface_module);
...
int primary_display_init(char *lcm_name, unsigned int lcm_fps)
{
...
#ifdef CONFIG_FOR_SOURCE_PQ
	/*create mira path cmdq handle */
	/* TODO: if we can just create this when need it? */
	ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_COLOR, &(pgc->cmdq_handle_sub_mem_config));
	if (ret != 0) {
		DISPERR("cmdqRecCreate FAIL, ret=%d\n", ret);
		ret = DISP_STATUS_ERROR;
		goto done;
	...
	if (disp_helper_get_option(DISP_HELPER_OPTION_USE_CMDQ)) {
		cmdqRecReset(pgc->cmdq_handle_sub_mem_config);
		/* cmdqRecWait(pgc->cmdq_handle_sub_mem_config, CMDQ_EVENT_DISP_WDMA1_EOF); */
	}

	ret = _build_path_memout_mira();

	if (disp_helper_get_option(DISP_HELPER_OPTION_USE_CMDQ)) {
		cmdqRecFlushAsync(pgc->cmdq_handle_sub_mem_config);
		cmdqRecReset(pgc->cmdq_handle_sub_mem_config);
	}
#endif
	...
	ret =
	    dpmgr_path_config(pgc->dpmgr_handle, data_config,
			      disp_helper_get_option(DISP_HELPER_OPTION_USE_CMDQ) ? pgc->cmdq_handle_config : NULL);
...
int primary_display_post_init(void)
{
	...
	if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
		...
		if (_need_register_eint()) {
			...
			if (node) {
				...
				if (request_irq
				    (irq, _esd_check_ext_te_irq_handler, IRQF_TRIGGER_NONE, "DSI_TE_1-eint", NULL)) {
		...
		primary_display_switch_dst_mode_task =
		    kthread_create(_disp_primary_path_switch_dst_mode_thread, NULL, "display_switch_dst_mode");
		...
		if (decouple_update_rdma_config_thread == NULL) {
			decouple_update_rdma_config_thread =
			    kthread_create(update_rdma_config_thread, NULL, "decouple_update_rdma_cfg");
	...
	if (primary_display_frame_update_task == NULL) {
		...
		primary_display_frame_update_task =
		    kthread_create(primary_display_frame_update_kthread, NULL, "frame_update_worker");
	...
	if (primary_display_is_video_mode() || (primary_display_get_lcm_max_refresh_rate() == 120))
		spm_enable_sodi(0);
	else
		spm_enable_sodi(1);
...
static unsigned int keeplow;
static int od_by_pass;
void primary_display_enable_keep_low(unsigned int flag)
{
	DISPCHECK("keeplow %u\n", flag);
	keeplow = flag;
}
...
int _display_set_lcm_refresh_rate(int fps)
{
	...
	int ret = 0;
	static cmdqRecHandle cmdq_handle;
	static cmdqRecHandle cmdq_pre_handle;
	DISPCHECK("_set lcm fps(%d)\n", fps);
	if (pgc->state == DISP_SLEEPED) {
	...
	if (fps == 120) {
		ret =
		    __primary_display_switch_path(DISP_SESSION_DECOUPLE_MODE,
						  MAKE_DISP_SESSION(DISP_SESSION_PRIMARY, 0), 0, DISP_USER_FPS);
		/* adjust fps and od */
		cmdqRecReset(cmdq_handle);
		_cmdq_insert_wait_frame_done_token_mira(cmdq_handle);
		...
		pgc->plcm->params->dsi.PLL_CLOCK = 280;
		...
		ret = cmdqRecWait(cmdq_handle, CMDQ_EVENT_MDP_DSI0_TE_SOF);
		disp_lcm_adjust_fps(cmdq_handle, pgc->plcm, fps);
		pgc->plcm->params->dsi.PLL_CLOCK = 150;
		dpmgr_path_ioctl(pgc->dpmgr_handle, cmdq_handle, DDP_PHY_CLK_CHANGE, &pgc->plcm->params->dsi.PLL_CLOCK);

		if (pgc->session_mode == DISP_SESSION_DECOUPLE_MODE) {
			cmdqRecReset(cmdq_pre_handle);
			cmdqRecWaitNoClear(cmdq_pre_handle, CMDQ_EVENT_DISP_WDMA0_EOF);
			disp_od_set_enabled(cmdq_pre_handle, 0);
			cmdqRecFlushAsync(cmdq_pre_handle);
			_cmdq_flush_config_handle_mira(cmdq_handle, 0);
		} else {
			disp_od_set_enabled(cmdq_handle, 0);
			_cmdq_flush_config_handle_mira(cmdq_handle, 0);
		}
		ret = __primary_display_switch_path(DISP_SESSION_DIRECT_LINK_MODE,
						  MAKE_DISP_SESSION(DISP_SESSION_PRIMARY, 0), 0, DISP_USER_FPS);
		od_need_start = 0;
	}
...
int primary_display_set_lcm_refresh_rate(int fps)
{
	...
	if (pgc->mhl_connect) {
		DISPCHECK("mhl connected, delay to adjust fps\n");
		pgc->delayed_fps = fps;
		ret = -1;
	} else {
...
int primary_display_suspend(void)
{
	...
	pgc->lcm_refresh_rate = 60;
#ifdef CONFIG_FOR_SOURCE_PQ
	/* power off miravision path */
	dpmgr_path_power_off(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
#endif
...
int primary_display_resume(void)
{
	...
	if (disp_helper_get_option(DISP_HELPER_OPTION_USE_CMDQ)) {
		...
		if (primary_display_is_decouple_mode()) {
			/*
			 * wdma is idle when initializing, so set token
			 * after trigger loop started
			 */
			cmdqCoreSetEvent(CMDQ_EVENT_DISP_WDMA0_EOF);
		}
	}
...
#ifdef CONFIG_FOR_SOURCE_PQ
	/* power on miravision path, and need to reconnecting */
	dpmgr_path_power_on(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
	dpmgr_path_connect(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
...
int primary_display_ipoh_hibernation_prepare(void)
{
	...
	if (disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)) {
		...
		primary_display_idlemgr_kick(__func__, 0);
		__primary_display_switch_path(DISP_SESSION_DIRECT_LINK_MODE, MAKE_DISP_SESSION(DISP_SESSION_PRIMARY, 0),
					      0, DISP_USER_NORMAL);
		_primary_path_unlock(__func__);
	}
...
int primary_display_ipoh_post_hibernation(void)
{
...
static int config_decouple_output_buffer(disp_path_handle disp_handle, cmdqRecHandle cmdq_handle)
{
...
static int trigger_decouple(void)
{
...
static int config_wdma_output(disp_path_handle disp_handle, cmdqRecHandle cmdq_handle, disp_mem_output_config *output)
{
	disp_ddp_path_config *pconfig = NULL;
...
int primary_display_config_output(disp_mem_output_config *output)
{
	...
	mem_config = *output;
	MMProfileLogEx(ddp_mmp_get_events()->primary_wdma_config, MMProfileFlagPulse, output->buff_idx,
...
static SVP_STATE svp_state = SVP_NORMAL;
static int svp_sum;

OPT_BACKUP svp_opt_backup[3] = {
	{DISP_HELPER_OPTION_IDLEMGR_SWTCH_DECOUPLE, 0},
	{DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE, 0},
	{DISP_HELPER_OPTION_AUTO_1L_DL, 0}
};
...
static int disp_enter_svp(SVP_STATE state)
{
	...
	if (state == SVP_IN_POINT) {
		for (i = 0; i < (sizeof(svp_opt_backup) / sizeof((svp_opt_backup)[0])); i++) {
			svp_opt_backup[i].value = disp_helper_get_option(svp_opt_backup[i].option);
			disp_helper_set_option(svp_opt_backup[i].option, 0);
		}

		if (primary_display_is_decouple_mode() && (!primary_display_is_mirror_mode())) {
			/* switch to DL */
			/*__primary_display_switch_mode(DISP_SESSION_DIRECT_LINK_MODE, pgc->session_id);*/
			__primary_display_switch_path(DISP_SESSION_DIRECT_LINK_MODE,
			pgc->session_id, 0, DISP_USER_SVP);
		}
		/*idlemgr_flag_backup = set_idlemgr(0, 0);*/
		/*we disable esd check when enter svp, and will be enable until resume*/
		primary_display_esd_check_enable(0);
	}
...
static int disp_leave_svp(SVP_STATE state)
{
	...
	if (state == SVP_EXIT_POINT) {
		...
		for (i = 0; i < 3; i++)
			disp_helper_set_option(svp_opt_backup[i].option, svp_opt_backup[i].value);
...
static int setup_disp_sec(disp_ddp_path_config *data_config, cmdqRecHandle cmdq_handle, int is_locked)
{
	int i, ret, has_sec_layer = 0;
		...
		if (need_dc) {
			...
			if (pgc->ovl_one_layer == 1) {
				...
				if (disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)) {
					_primary_path_unlock(__func__);
					mmdvfs_set_step(SMI_BWC_SCEN_UI_IDLE, MMDVFS_VOLTAGE_HIGH);
					__primary_display_switch_path(DISP_SESSION_DECOUPLE_MODE, session_id, 1,
								      DISP_USER_LAYER);
					mmdvfs_set_step(SMI_BWC_SCEN_UI_IDLE, MMDVFS_VOLTAGE_DEFAULT);
					_primary_path_lock(__func__);

				} else {
					__primary_display_switch_path(DISP_SESSION_DECOUPLE_MODE, session_id, 0,
								      DISP_USER_LAYER);
				}
			}
		} else {
			if (one_layer_cnt >= one_layer_dl_condition) {
				if (pgc->ovl_one_layer == 0) {
					...
					pgc->ovl_one_layer = 1;
					__primary_display_switch_path(DISP_SESSION_DIRECT_LINK_MODE, session_id, 0,
								      DISP_USER_LAYER);
				}
			} else {
				one_layer_cnt++;
...
int primary_display_config_input_multiple(primary_disp_input_config *input)
{
	...
	/* Check path config first, __primary_display_switch_path() will restore gce handle token,
	so it's safe for original ovl config to use */
		...
		for (i = 0; i < HW_OVERLAY_COUNT; i++) {
			...
			if (input[i].dirty) {
				dprec_logger_start(DPREC_LOGGER_PRIMARY_CONFIG,
						   input[i].layer | (input[i].layer_en << 16), input[i].addr);
			...
			if ((ovl_cfg->layer == 0) && (!primary_display_is_decouple_mode()))
				update_frm_seq_info(ovl_cfg->addr,
						    ovl_cfg->src_x * 4 + ovl_cfg->src_y * ovl_cfg->src_pitch,
		...
		for (i = 0; i < HW_OVERLAY_COUNT; i++) {
			...
			if (input[i].dirty) {
				...
				if (input_cfg->buffer_source == DISP_BUFFER_ALPHA
				    || input_cfg->layer_en == 0 || cur_fence == -1) {
	...
	/*update hwc refresh information */
	pgc->hwc_refresh_cnt++;
...
static int Panel_Master_primary_display_config_dsi(const char *name, UINT32 config_value)
{
	...
	if (!strcmp(name, "PM_CLK")) {
		...
		module = pgc->plcm->params->lcm_if == LCM_INTERFACE_DSI_DUAL ? DISP_MODULE_DSIDUAL :
			pgc->plcm->params->lcm_if == LCM_INTERFACE_DSI0 ? DISP_MODULE_DSI0 :
			pgc->plcm->params->lcm_if == LCM_INTERFACE_DSI1 ? DISP_MODULE_DSI1 :
			-1;
		DSI_ChangeClk(DISP_MODULE_DSIDUAL, config_value);
...
#ifdef CONFIG_FOR_SOURCE_PQ
static int _need_apply_on_pq_path(unsigned int cmd)
{
	int ret = 0;
	switch (cmd) {
...
int primary_display_user_cmd(unsigned int cmd, unsigned long arg)
{
	...
	if (pgc->state != DISP_SLEEPED) {
		if (pgc->session_mode == DISP_SESSION_DECOUPLE_MODE) {
			if (disp_helper_get_option(DISP_HELPER_OPTION_DECOUPLE_UFOE_MODE)) {
				/*pq in pre path */
				pq_place = 1;
				ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_MEMOUT, &handle);
				if (ret != 0) {
					DISPERR("user_cmd fail to create mem_out handle\n");
					handle = NULL;
				}
				cmdqRecReset(handle);
				...
				ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &handle);
				if (ret != 0) {
					DISPERR("user_cmd fail to create primary handle\n");
					handle = NULL;
				}
				cmdqRecReset(handle);
				_cmdq_insert_wait_frame_done_token_mira(handle);
...
#ifdef CONFIG_FOR_SOURCE_PQ
		ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_COLOR, &sub_handle);
		cmdqRecReset(sub_handle);
		/* cmdqRecWaitNoClear(sub_handle, CMDQ_EVENT_DISP_WDMA1_EOF); */
		sub_size = cmdqRecGetInstructionCount(sub_handle);
#endif
	}
	_primary_path_unlock(__func__);

#ifdef CONFIG_FOR_SOURCE_PQ
	if (_need_apply_on_pq_path(cmd))
		ret = dpmgr_path_user_cmd(pgc->dpmgr_sub_mem_path_handle, cmd, arg, sub_handle);
	else
		ret = dpmgr_path_user_cmd(pgc->dpmgr_handle, cmd, arg, handle);

	if (sub_handle) {
		if (cmdqRecGetInstructionCount(sub_handle) > sub_size)
			cmdqRecFlushAsync(sub_handle);

		cmdqRecDestroy(sub_handle);
	}
#else
	ret = dpmgr_path_user_cmd(pgc->dpmgr_handle, cmd, arg, handle);
#endif
				...
				if (disp_helper_get_option(DISP_HELPER_OPTION_DECOUPLE_UFOE_MODE)) {
					if (pgc->session_mode == DISP_SESSION_DECOUPLE_MODE) {
						if (pq_place == 1)
							...
							DISPCHECK("ignore user_command when mode switch!!\n");
					} else {
						if (pq_place == 2)
							_cmdq_flush_config_handle_mira(handle, 0);
						else
...
static DISP_MODE select_mode_from_feature(void)
{
	...
	if (disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)) {
		if (pgc->ovl_one_layer == 1 || pgc->mmdvfs_power_state == 1)
			return DISP_SESSION_DIRECT_LINK_MODE;
		else
			return DISP_SESSION_DECOUPLE_MODE;
	} else {
...
static int __primary_display_switch_mode(int sess_mode, unsigned int session)
{
	int sw_only = 0;

	MMProfileLogEx(ddp_mmp_get_events()->primary_switch_mode, MMProfileFlagStart, pgc->session_mode, sess_mode);

	/* switch in suspend should be handled first */
	if (pgc->state == DISP_SLEEPED) {
		DISPCHECK("[DISPMODE]require %s to %s in suspend state!!!\n",
			session_mode_spy(pgc->session_mode), session_mode_spy(sess_mode));
		sw_only = 1;
		/* we don't switch path when path is sleep. after resume we do path switch according select mode. */
		return 0;
	}

	DISPCHECK("[DISPMODE] %s to %s\n", session_mode_spy(pgc->session_mode), session_mode_spy(sess_mode));
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE && sess_mode == DISP_SESSION_DECOUPLE_MODE) {
		...
		if (disp_helper_get_option(DISP_HELPER_OPTION_TWO_PIPE_INTERFACE_PATH)) {
			...
			/* ufoe decouple */
			if (disp_helper_get_option(DISP_HELPER_OPTION_DECOUPLE_UFOE_MODE))
				pgc->decouple_type = DECOUPLE_UFOE;

			DL_switch_to_DC_fast(sw_only);
		}
		pgc->session_mode = sess_mode;
		MMProfileLogEx(ddp_mmp_get_events()->primary_switch_mode, MMProfileFlagPulse, pgc->session_mode,
			       sess_mode);
...
static int __primary_display_switch_path(int request_mode, unsigned int session, int need_lock, enum DISP_USER user)
{
	int dst_mode = request_mode;
	int tmp_mode = request_mode;
	if (need_lock)
		_primary_path_lock(__func__);

	if (disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)
	    || (primary_display_get_lcm_max_refresh_rate() == 120)) {
		dst_mode = select_session_mode(request_mode, user);
		DISPDBG("[DISPMODE]: cur %s, req %s, res %s by user(%d)\n",
			session_mode_spy(pgc->session_mode), session_mode_spy(request_mode), session_mode_spy(dst_mode),
			user);
		if (pgc->session_mode != dst_mode) {
			if (disp_helper_get_option(DISP_HELPER_OPTION_DECOUPLE_UFOE_MODE)) {
				/*decouple mirror <--> decoule ufoe need temp mode directlink */
				if ((pgc->session_mode == DISP_SESSION_DECOUPLE_MODE
				     && dst_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE)
				    || (pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE
					&& dst_mode == DISP_SESSION_DECOUPLE_MODE)) {
					/* swith to directlink */
					__primary_display_switch_mode(DISP_SESSION_DIRECT_LINK_MODE, session);
					/*re-select dst mode */
					tmp_mode = dst_mode;
					dst_mode = select_session_mode(tmp_mode, user);
					DISPCHECK("[DISPMODE]:reselect cur %s, req %s, res %s by user(%d)\n",
						session_mode_spy(pgc->session_mode), session_mode_spy(tmp_mode),
						session_mode_spy(dst_mode), user);
				}
			}
			__primary_display_switch_mode(dst_mode, session);
		}
	} else {
		if (pgc->session_mode != dst_mode)
			__primary_display_switch_mode(dst_mode, session);
...
int primary_display_switch_mode(int sess_mode, unsigned int session, int force)
{
	...
	_primary_path_lock(__func__);
	primary_display_idlemgr_kick(__func__, 0);
	ret = __primary_display_switch_path(sess_mode, session, 0, DISP_USER_HWC);
	_primary_path_unlock(__func__);

	return ret;
}

int primary_display_switch_path_test(int sess_mode)
{
	_primary_protect_mode_switch();
	_primary_path_lock(__func__);
	pgc->test_mode = sess_mode;
	__primary_display_switch_path(sess_mode, MAKE_DISP_SESSION(DISP_SESSION_PRIMARY, 0), 0, DISP_USER_TEST);
	_primary_path_unlock(__func__);
	return 0;
}

int mhl_pluged(int in)
{
#ifdef CONFIG_MTK_DISPLAY_120HZ_SUPPORT
	_primary_protect_mode_switch();
	_primary_path_lock(__func__);
	...
	if (pgc->state == DISP_SLEEPED) {
		_primary_path_unlock(__func__);
		DISPCHECK("Sleep State mhl_pluged\n");
		return -1;
	}
	pgc->mhl_connect = in;
	if (in) {
		DISPCHECK("mhl_pluged in, current fps %d\n", pgc->lcm_refresh_rate);
		pgc->delayed_fps = pgc->lcm_refresh_rate;
		_display_set_lcm_refresh_rate(60);
	} else {
		DISPCHECK("mhl_pluged out, to fps %d\n", pgc->lcm_refresh_rate);
		pgc->mhl_connect = in;
		_display_set_lcm_refresh_rate(pgc->delayed_fps);
	}
	_primary_path_unlock(__func__);
#endif
	return 0;
...
int primary_display_switch_mode_for_mmdvfs(int sess_mode, unsigned int session, int blocking)
{
	int ret = 0;
	if (disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)) {
		DISPDBG("[mmdvfs] switch to mode %d begin\n", sess_mode);
		_primary_protect_mode_switch();
		_primary_path_lock(__func__);
		...
		if (sess_mode == DISP_SESSION_DIRECT_LINK_MODE) {
			...
			DISPCHECK("[mmdvfs]power state 1\n");
		} else if (sess_mode == DISP_SESSION_DECOUPLE_MODE) {
			pgc->mmdvfs_power_state = 0;
			DISPCHECK("[mmdvfs]power state 0, display suspend %d\n", pgc->state == DISP_SLEEPED);
			if (pgc->state == DISP_ALIVE) {
				primary_display_idlemgr_kick(__func__, 0);
				ret = __primary_display_switch_path(sess_mode, session, 0, DISP_USER_MMDVFS);
			}
		} else {
			DISPERR("invalid sess_mode request by mmdvfs[%s]\n", disp_session_mode_spy(sess_mode));
		}
		_primary_path_unlock(__func__);
		DISPDBG("[mmdvfs] switch to mode %d end\n", sess_mode);
	} else {
...
{
	int ret = 0;
	DISPCHECK("%s primary display's path cg\n", enable ? "enable" : "disable");
	_primary_path_lock(__func__);
...
int primary_display_capture_framebuffer_ovl(unsigned long pbuf, unsigned int format)
{
	...
	ret =
	    m4u_alloc_mva(m4uClient, M4U_PORT_DISP_WDMA0, pbuf, NULL, buffer_size, M4U_PROT_READ | M4U_PROT_WRITE, 0,
			  &mva);
	if (ret != 0) {
...
int primary_display_capture_framebuffer(unsigned long pbuf)
{
	...
	void *fbv = 0;
	DISPMSG("w_res=%d, h_yres=%d, pixel_bpp=%d, w_fb=%d, fbsize=%d, fbaddress=0x%08x\n", w_xres, h_yres, pixel_bpp,
		w_fb, fbsize, fbaddress);
	fbv = ioremap(fbaddress, fbsize);
	DISPMSG("w_xres = %d, h_yres = %d, w_fb = %d, pixel_bpp = %d, fbsize = %d, fbaddress = 0x%08x\n", w_xres,
		h_yres, w_fb, pixel_bpp, fbsize, fbaddress);
	if (!fbv) {
		DISPCHECK("[FB Driver], Unable to allocate memory for frame buffer: address=0x%08x, size=0x%08x\n",
			fbaddress, fbsize);
		return -1;
	}
	...
	for (i = 0; i < h_yres; i++) {
		...
		memcpy((void *)(pbuf + i * w_xres * pixel_bpp), (void *)(fbv + i * w_fb * pixel_bpp),
		       w_xres * pixel_bpp);
	}
	DISPCHECK("capture framebuffer cost %ldus\n", get_current_time_us() - ttt);
	iounmap((void *)fbv);
#endif
	return -1;
}
...
UINT32 DISP_GetFBRamSize(void)
{
	return ALIGN_TO(DISP_GetScreenWidth(), MTK_FB_ALIGNMENT) *
	    ALIGN_TO(DISP_GetScreenHeight(), MTK_FB_ALIGNMENT) * ((DISP_GetScreenBpp() + 7) >> 3) * DISP_GetPages();
}
...
{
	int ret = 0;
	DISPMSG("disphal_allocate_fb, pa=%pa\n", &pa_start);
	*va = (unsigned long)ioremap_nocache(pa_start, pa_end - pa_start + 1);
	DISPMSG("disphal_allocate_fb, pa=%pa, va=0x%lx\n", &pa_start, *va);
	...
	if (disp_helper_get_option(DISP_HELPER_OPTION_USE_M4U)) {
		...
		*mva = pa_start & 0xffffffffULL;
		...
		DISPMSG("[DISPHAL] FB MVA is 0x%lx PA is %pa\n", *mva, &pa_start);
