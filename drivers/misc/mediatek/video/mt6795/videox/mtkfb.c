...
static int mtkfb_setcolreg(u_int regno, u_int red, u_int green, u_int blue, u_int transp, struct fb_info *info)
{
	...
	case 32:
		/* ARGB8888 */
		((u32 *) (info->pseudo_palette))[regno] =
		    (0xff000000) | ((red & 0xFF00) << 8) | ((green & 0xFF00)) | ((blue & 0xFF00) >> 8);
		break;
...
#if 1
	DISPDBG("_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%x, paddr=0x%x,",
		dst->layer, dst->layer_en, dst->buff_idx, (unsigned int)(dst->vaddr), (unsigned int)(dst->addr));
	DISPDBG(" src fmt=%u, dst fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
		src->src_fmt, dst->fmt, dst->src_pitch, dst->src_x, dst->src_y, dst->src_w, dst->src_h);
...
#if 1
	DISPMSG("_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%lx, paddr=0x%lx, fmt=%u,",
		dst->layer, dst->layer_en, dst->buff_idx, dst->addr, dst->vaddr, dst->fmt);
	DISPMSG(" pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	DISPDBG("xoffset=%u, yoffset=%u, xres=%u, yres=%u, xresv=%u, yresv=%u\n", var->xoffset,
		var->yoffset, info->var.xres, info->var.yres, info->var.xres_virtual, info->var.yres_virtual);
...
#if 0
	{
		...
		DISPMSG("fb dump: 0x%08x, 0x%08x, 0x%08x, 0x%08x\n", *(unsigned int *)va,
				*(unsigned int *)(va + 4), *(unsigned int *)(va + 8), *(unsigned int *)(va + 0xC));
...
static int mtkfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case MTKFB_POWEROFF:
	{
		...
		MTKFB_LOG("[FB Driver] enter MTKFB_POWEROFF\n");
		/* FIXME: temp for AOSP */
		/* cci400_sel_for_ddp(); */
		...
		MTKFB_LOG("[FB Driver] leave MTKFB_POWEROFF\n");
	...
	case MTKFB_POWERON:
	{
		...
		MTKFB_LOG("[FB Driver] enter MTKFB_POWERON\n");
		primary_display_resume();
		MTKFB_LOG("[FB Driver] leave MTKFB_POWERON\n");
	...
	case MTKFB_GET_POWERSTATE:
	{
		...
		if (primary_display_is_sleepd())
			...
			if (copy_to_user(argp, &power_state, sizeof(power_state))) {
				DISPMSG("MTKFB_GET_POWERSTATE failed\n");
				return -EFAULT;
			}
	...
	case MTKFB_SET_OVERLAY_LAYER:
	{
		...
		if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
			...
			if (primary_display_is_sleepd()) {
					DISPMSG("[FB] error, set overlay in early suspend ,skip!\n");
	...
	case MTKFB_SET_VIDEO_LAYERS:
	{
		...
		if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
			MTKFB_LOG("[FB]: copy_from_user failed! line:%d\n", __LINE__);
				MMProfileLogMetaString(MTKFB_MMP_Events.SetOverlayLayers, MMProfileFlagEnd,
			...
			/* MMProfileFlagEnd, layerInfo, struct mmp_fb_overlay_layers); */
	...
	case MTKFB_META_SHOW_BOOTLOGO:
	{
		...
		input.addr =
			fbdev->fb_pa_base +
			    (ALIGN_TO(MTK_FB_XRES, MTK_FB_ALIGNMENT) * ALIGN_TO(MTK_FB_YRES, MTK_FB_ALIGNMENT) * 4);
		input.src_x = 0;
		input.src_y = 0;
...
/* #if defined (MTK_FB_SYNC_SUPPORT) */
	...
	case MTKFB_SET_MULTIPLE_LAYERS:
	{
		...
		if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
			MTKFB_LOG("[FB]: copy_from_user failed! line:%d\n", __LINE__);
				MMProfileLogMetaString(MTKFB_MMP_Events.SetMultipleLayers, MMProfileFlagEnd,
						       "Copy_from_user failed!");
			r = -EFAULT;
		} else {
			int32_t i;
			...
			for (i = 0; i < HW_OVERLAY_COUNT; ++i) {
				DISPMSG("mslayer(0x%08x) bidx (%d)\n",
						(layerInfo[i].layer_id << 24 | layerInfo[i].layer_enable),
						layerInfo[i].next_buff_idx);
				if (layerInfo[i].layer_id >= HW_OVERLAY_COUNT)
					continue;

#if 0
				if (layerInfo[i].layer_enable
					    && (layerInfo[i].next_buff_idx == cached_layer_config[i].buff_idx)) {
...
#endif
				if (layerInfo[i].layer_enable
					    && (layerInfo[i].next_buff_idx <= mtkfb_fence_timeline_index(i))) {
						DISPERR("layerId(%d) HWC set the old buffer(%d),timeline_idx(%d)!\n",
							layerInfo[i].layer_id, layerInfo[i].next_buff_idx,
							mtkfb_fence_timeline_index(i));
					continue;
				}

				if (is_early_suspended) {
						DISPERR("in early suspend layer(0x%x),idx(%d),time_idx(%d)!\n",
							layerInfo[i].layer_id << 16 | layerInfo[i].layer_enable,
							layerInfo[i].next_buff_idx, mtkfb_fence_timeline_index(i));
...
#endif
	default:
		DISPMSG("mtkfb_ioctl Not support, info=0x%p, cmd=0x%08x, arg=0x%08lx\n", info, (unsigned int)cmd, arg);
		return -EINVAL;
	}
}
...
static int mtkfb_compat_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	DISPFUNC();
	MMProfileLogEx(MTKFB_MMP_Events.IOCtrl, MMProfileFlagPulse, _IOC_NR(cmd), arg);
	DISPMSG("mtkfb_compat_ioctl, info=%p, cmd nr=0x%08x, cmd size=0x%08x,arg=0x%lx\n", info,
	...
	case COMPAT_MTKFB_GET_POWERSTATE:
	{
		...
		if (put_user(power_state, data32)) {
			...
			DISPMSG("MTKFB_GET_POWERSTATE success %d\n", power_state);
		return 0;
	}
...
static void _mtkfb_draw_block(unsigned long addr, unsigned int x, unsigned int y, unsigned int w, unsigned int h,
			      unsigned int color)
{
	int i = 0;
	int j = 0;
...
static int _mtkfb_internal_test(unsigned long va, unsigned int w, unsigned int h)
{
	...
	for (j = 0; j < 10; j++) {
		for (i = 0; i < 64; i++) {
			memcpy((void *)(va + 720 * 1280 * 3 + 360 * 3 + j * 720 * 70 * 3 + 720 * 3 * i),
...
int __parse_tag_videolfb_extra(struct device_node *node)
{
	...
	prop = of_get_property(node, "atag,videolfb-fb_base_h", NULL);
	if (!prop)
		return -1;
	fb_base_h = of_read_number(prop, 1);

	prop = of_get_property(node, "atag,videolfb-fb_base_l", NULL);
	if (!prop)
		return -1;
	...
	prop = of_get_property(node, "atag,videolfb-islcmfound", NULL);
	if (!prop)
		return -1;
	islcmconnected = of_read_number(prop, 1);

	prop = of_get_property(node, "atag,videolfb-fps", NULL);
	if (!prop)
		return -1;
	...
	prop = of_get_property(node, "atag,videolfb-vramSize", NULL);
	if (!prop)
		return -1;
...
int mtkfb_ipoh(struct notifier_block *nb, unsigned long val, void *ign)
{
	...
	case PM_HIBERNATION_PREPARE:
		primary_display_ipoh_hibernation_prepare();
		return NOTIFY_DONE;
	case PM_RESTORE_PREPARE:
		primary_display_ipoh_restore_prepare();
		return NOTIFY_DONE;
	case PM_POST_HIBERNATION:
		primary_display_ipoh_post_hibernation();
		return NOTIFY_DONE;
	}
	return NOTIFY_OK;
}
...
static const struct platform_driver mtkfb_driver = {
	.driver = {
		.name = MTKFB_DRIVER,
#ifdef CONFIG_PM
		.pm = &mtkfb_pm_ops,
...
int mtkfb_get_debug_state(char *stringbuf, int buf_len)
{
	...
	len +=
	    scnprintf(stringbuf + len, buf_len - len, "|xres=%d, yres=%d,bpp=%d,pages=%d,linebytes=%d,total size=%d\n",
		      MTK_FB_XRES, MTK_FB_YRES, MTK_FB_BPP, MTK_FB_PAGES, MTK_FB_LINE, MTK_FB_SIZEV);
