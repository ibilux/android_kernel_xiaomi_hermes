...
#define PWM_DBG(fmt, arg...) pr_debug("[PWM] " fmt "\n", ##arg)
	...
	if (cust_led_list) {
		...
		if ((strcmp(cust->name, "lcd-backlight") == 0)
			...
			if (config_data->clock_source >= 0 && config_data->clock_source <= 3) {
				...
				PWM_DBG("disp_pwm_init : CLK_CFG_1 0x%x => 0x%x", regVal, DISP_REG_GET(CLK_CFG_1));
			}
			...
			pwm_div &= 0x3FF;
			PWM_DBG("disp_pwm_init : PWM config data (%d,%d)", config_data->clock_source, config_data->div);
		}
	}
...
static void disp_pwm_set_enabled(cmdqRecHandle cmdq, disp_pwm_id_t id, int enabled)
{
	...
	if (enabled) {
		if (!disp_pwm_is_enabled(id)) {
			...
			PWM_DBG("disp_pwm_set_enabled: PWN_EN = 0x1");
...
int disp_pwm_set_max_backlight(disp_pwm_id_t id, unsigned int level_1024)
{
	...
	PWM_DBG("disp_pwm_set_max_backlight(id = 0x%x, level = %u)", id, level_1024);
