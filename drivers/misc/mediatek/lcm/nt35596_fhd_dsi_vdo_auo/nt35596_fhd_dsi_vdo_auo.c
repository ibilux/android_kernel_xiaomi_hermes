...
#ifndef BUILD_LK
#define AUO_START_DELAY		15
#define AUO_DATA_DELAY		10
#define AUO_END_L_DELAY		15
#define AUO_END_H_DELAY		350
static DEFINE_SPINLOCK(g_auo_data_lock);

#define KTD3116_AUO_EXPRESSWRITE_START()\
{\
	mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ONE);\
	UDELAY(AUO_START_DELAY);\
}

#define KTD3116_AUO_EXPRESSWRITE_WRITE(data)\
{\
	int i;\
	for(i = 7; i >= 0; i--)\
	{\
		...
		{\
			...
			UDELAY(AUO_DATA_DELAY);\
			mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ONE);\
			UDELAY(3 * AUO_DATA_DELAY);\
		}\
		else\
		{\
			mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ZERO);\
...
#define KTD3116_AUO_EXPRESSWRITE_END()\
{\
	mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ZERO);\
	UDELAY(AUO_END_L_DELAY);\
	mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ONE);\
	UDELAY(AUO_END_H_DELAY);\
}

static void KTD3116_Auo_SendData(unsigned char data)
{
	spin_lock_irq(&g_auo_data_lock);
	KTD3116_AUO_EXPRESSWRITE_START();
	KTD3116_AUO_EXPRESSWRITE_WRITE(data);
	KTD3116_AUO_EXPRESSWRITE_END();
	spin_unlock_irq(&g_auo_data_lock);
}
#endif
...
static void lcm_setbacklight_cmdq(void* handle,unsigned int level)
{
	...
	first_vlue = value;
#ifndef BUILD_LK
	printk("%s, auo, line=%d, value=%d, first_vlue=%d, second_vlue=%d\n", __FUNCTION__, __LINE__, value,  first_vlue, second_vlue);
#endif
