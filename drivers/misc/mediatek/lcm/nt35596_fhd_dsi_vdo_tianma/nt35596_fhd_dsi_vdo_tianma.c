...
#if 1	//modify by lizhiye, for gamma shift. 20151225
	{0xFF, 1, {0x01}},
	{0xFB, 1, {0x01}},
	{0x00, 1, {0x01}},
	{0x01, 1, {0x55}},
...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	params->dsi.vertical_frontporch					= 22;	///4
...
#ifndef BUILD_LK
#define TIANMA_START_DELAY		15
#define TIANMA_DATA_DELAY		10
#define TIANMA_END_L_DELAY		15
#define TIANMA_END_H_DELAY		350
static DEFINE_SPINLOCK(g_tianma_data_lock);

#define KTD3116_TIANMA_EXPRESSWRITE_START()\
{\
	mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ONE);\
	UDELAY(TIANMA_START_DELAY);\
}

#define KTD3116_TIANMA_EXPRESSWRITE_WRITE(data)\
{\
	int i;\
	for(i = 7; i >= 0; i--)\
	{\
		...
		{\
			...
			UDELAY(TIANMA_DATA_DELAY);\
			mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ONE);\
			UDELAY(3 * TIANMA_DATA_DELAY);\
		}\
		else\
		{\
			mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ZERO);\
...
#define KTD3116_TIANMA_EXPRESSWRITE_END()\
{\
	mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ZERO);\
	UDELAY(TIANMA_END_L_DELAY);\
	mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ONE);\
	UDELAY(TIANMA_END_H_DELAY);\
}

static void KTD3116_Tianma_SendData(unsigned char data)
{
	spin_lock_irq(&g_tianma_data_lock);
	KTD3116_TIANMA_EXPRESSWRITE_START();
	KTD3116_TIANMA_EXPRESSWRITE_WRITE(data);
	KTD3116_TIANMA_EXPRESSWRITE_END();
	spin_unlock_irq(&g_tianma_data_lock);
}
#endif
...
static void lcm_setbacklight_cmdq(void* handle,unsigned int level)
{
	...
	else
	{
		...
		if(value >= 32)
		{	
			if((first_vlue >= 32) && (second_vlue < 32))
			{
				KTD3116_Tianma_SendData(0);
			}
			else if((first_vlue >= 32) && (second_vlue >= 32))
			{
				mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ONE);
			}
			lcm_backlight_level_setting[2].para_list[0] = value;
