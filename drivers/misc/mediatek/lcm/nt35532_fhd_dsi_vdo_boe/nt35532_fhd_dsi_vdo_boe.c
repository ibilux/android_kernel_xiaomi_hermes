...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	// 1 Every lane speed
#if 1
	params->dsi.PLL_CLOCK=475;	//485;475;462
#else
...
#ifndef BUILD_LK
#define BOE_START_DELAY		15
#define BOE_DATA_DELAY		10
#define BOE_END_L_DELAY		15
#define BOE_END_H_DELAY		350
static DEFINE_SPINLOCK(g_boe_data_lock);

#define KTD3116_BOE_EXPRESSWRITE_START()\
{\
	mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ONE);\
	UDELAY(BOE_START_DELAY);\
}

#define KTD3116_BOE_EXPRESSWRITE_WRITE(data)\
{\
	int i;\
	for(i = 7; i >= 0; i--)\
	{\
		...
		{\
			...
			UDELAY(BOE_DATA_DELAY);\
			mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ONE);\
			UDELAY(3 * BOE_DATA_DELAY);\
		}\
		else\
		{\
			mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ZERO);\
...
#define KTD3116_BOE_EXPRESSWRITE_END()\
{\
	mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ZERO);\
	UDELAY(BOE_END_L_DELAY);\
	mt_set_gpio_out(GPIO_LCD_BL_EN_PIN, GPIO_OUT_ONE);\
	UDELAY(BOE_END_H_DELAY);\
}

static void KTD3116_Boe_SendData(unsigned char data)
{
	spin_lock_irq(&g_boe_data_lock);
	KTD3116_BOE_EXPRESSWRITE_START();
	KTD3116_BOE_EXPRESSWRITE_WRITE(data);
	KTD3116_BOE_EXPRESSWRITE_END();
	spin_unlock_irq(&g_boe_data_lock);
}
#endif
