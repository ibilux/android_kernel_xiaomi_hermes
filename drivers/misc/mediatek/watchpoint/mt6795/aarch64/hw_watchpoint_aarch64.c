...
{
	wp_flag++;
	pr_debug("[MTK WP] Access my data from an instruction at 0x%lx\n", addr);
	return 0;
}

int my_wp_handler2(unsigned long addr)
{
	pr_debug("[MTK WP] In my_wp_handler2 Access my data from an instruction at 0x%lx\n", addr);
	/* trigger exception */
	return 0;
}
...
{
	int i;
	init_wp_event(&wp_event, (unsigned long)&my_watch_data,
			(unsigned long)&my_watch_data, WP_EVENT_TYPE_ALL, my_wp_handler1);

	err = add_hw_watchpoint(&wp_event);
	if (err) {
		pr_err("[MTK WP] add hw watch point failed...\n");
...
void wp_test2(void)
{
	...
	for (i = 1; i < num_possible_cpus(); i++) {
		...
		if (ret != 0)
			pr_err("[MTK WP] cpu %d power down failed\n", i);

		if (!cpu_online(i))
			pr_err("[MTK WP] cpu %d already power down\n", i);
	}
	for (i = 0; i < num_possible_cpus(); i++) {
		ret = cpu_up(i);
		if (ret != 0)
			pr_err("[MTK WP] cpu %d power up failed\n", i);

		if (cpu_online(i))
			pr_err("[MTK WP] cpu %d already power up\n", i);
		...
		if (ret == 0)
			...
			pr_err("[MTK WP] cpu %d, wite data in specific address failed\n", i);
	}

	if (wp_flag == num_possible_cpus())
		pr_debug("[MTK WP] Watchpoint item2 verfication pass 0x%x\n", wp_flag);
	else
		pr_err("[MTK WP] Watchpoint item2 verfication failed 0x%x\n", wp_flag);
}
...
void wp_test3(void)
{
	...
	if (wp_flag == num_possible_cpus())
		...
		pr_err("[MTK WP] Watchpoint item3 verfication failed 0x%x\n", wp_flag);
...
#endif
	pr_debug("[MTK WP]Authentication Interface, status 0x%x, wp_enable 0x%x\n", auth_status, wp_enable);
	return wp_enable;
};
	...
	for (i = 0; i < num_possible_cpus(); i++) {
		if (cpu_online(i)) {
			...
			pr_debug("[MTK WP] cpu %d, EDSCR &0x%lx= 0x%x\n", i,
					((vmalloc_to_pfn((void *)wp_tracer.debug_regs[i]) << 12) + EDSCR), args);
			if (args & HDE) {
				pr_err("[MTK WP] halting debug mode enabled. Unable to access hardware resources.\n");
				return -EPERM;
			}
...
void reset_watchpoint(void)
{
	...
	for (j = 0; j <  num_possible_cpus(); j++) {
		if (cpu_online(j)) {
			...
			if (args & HDE) {
				pr_err("[MTK WP]Reset flow halting debug mode enabled. Unable to reset hardware resources.\n");
				cs_cpu_write(wp_tracer.debug_regs[j], EDLAR, ~UNLOCK_KEY);
				cs_cpu_write(wp_tracer.debug_regs[j], OSLAR_EL1, UNLOCK_KEY);
				return;
			}
...
int add_hw_watchpoint(struct wp_event *wp_event)
{
	...
	pr_debug("[MTK WP] Hotplug disable\n");
	cpu_hotplug_disable();
	pr_debug("[MTK WP] Add watchpoint %d at address 0x%lx\n", i, wp_tracer.wp_events[i].virt);
	for (j = 0; j <  num_possible_cpus(); j++) {
		if (cpu_online(j)) {
	...
	for (j = 0; j < num_possible_cpus(); j++) {
		if (cpu_online(j))
			cs_cpu_write(wp_tracer.debug_regs[j], DBGWCR + (nr_wp << 4),
				cs_cpu_read(wp_tracer.debug_regs[j], DBGWCR + (nr_wp << 4)) & enable);
		else
