...
static void ich_pre_init(void)
{
    ...
    battery_log(BAT_LOG_FULL,"[ich_pre_init] Reg[0x%x]=0x%x\n", 0x8078, upmu_get_reg_value(0x8078));
}
...
{
    battery_log(BAT_LOG_FULL,"[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x\n",
    ...
    battery_log(BAT_LOG_FULL,"[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x\n",
...
{
    battery_log(BAT_LOG_CRTI,"[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x\n",
        0x805E, upmu_get_reg_value(0x805E),
        0x8056, upmu_get_reg_value(0x8056),
        0x80E2, upmu_get_reg_value(0x80E2),
        0x8062, upmu_get_reg_value(0x8062),
...
static void check_sw_recharge(void)
{
    ...
    if(g_sw_rechr_flag==0)
    {
        ...
        battery_log(BAT_LOG_FULL,"[check_sw_recharge] read RGS_CHARGE_COMPLETE_DET=%d\n", is_charge_complete);
    }
    ...
    battery_log(BAT_LOG_FULL,"[check_sw_recharge] Reg[0x%x]=0x%x,Reg[0x%x]=0x%x\n",
        0x8D30, upmu_get_reg_value(0x8D30), 0x8D34, upmu_get_reg_value(0x8D34) );
    battery_log(BAT_LOG_CRTI,"[check_sw_recharge] volt_vbat=%d, g_sw_rechr_vlot=%d, g_chr_complete_timer=%d, g_sw_rechr_flag=%d\n",
...
void set_cv_volt(void)
{
    ...
    if(is_m3_en==1)
    {
        battery_log(BAT_LOG_FULL,"[set_cv_volt] RGS_M3_EN=%d, set CV to high\n", is_m3_en);
    ...
    {
        battery_log(BAT_LOG_CRTI,"[set_cv_volt] RGS_M3_EN=%d, can not set CV to high\n", is_m3_en);
    }
}
...
void swchr_hw_init(void)
{
	...
	mt6332_upmu_set_rg_vin_dpm_mode_off(0x1);     /*[4], 1->0, Tim, 20140328*/
    //Reg[0x8166]
    ...
    #ifdef CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT
    {
        ...
        if(is_m3_en==1)
        {
            ...
            battery_log(BAT_LOG_CRTI,"[swchr_hw_init] mt6332_upmu_set_rg_pwm_en(1)\n");
        }
		else
		{
		    battery_log(BAT_LOG_CRTI,"[swchr_hw_init] mt6332_upmu_set_rg_pwm_en(0)\n");
	    }
    }
    #endif
...
static kal_uint32 charging_enable(void *data)
{
    ...
    pmic_read_interface(0x805E, &is_m3_en, 0x1, 2); //RGS_M3_EN
    battery_log(BAT_LOG_FULL,"[charging_enable] read RGS_M3_EN=%d\n", is_m3_en);
...
static void swchr_flow_main(kal_uint32 chr_cur_val)
{
    ...
    battery_log(BAT_LOG_FULL,"[swchr_flow_main] read RGS_PWM_EN=%d\n", is_pwm_en);

    pmic_read_interface(0x805E, &is_m3_en, 0x1, 2); //RGS_M3_EN
    battery_log(BAT_LOG_FULL,"[swchr_flow_main] read RGS_M3_EN=%d\n", is_m3_en);

    //Notmal mode charging, ex. CC and CV
    if(is_m3_en==1)
    {
        battery_log(BAT_LOG_FULL, "[swchr_flow_main] m3_en==%d (0x%x)\n", is_m3_en, get_pmic_mt6332_cid());
    ...
    {
        battery_log(BAT_LOG_CRTI, "[swchr_flow_main] m3_en==0\n");
        ...
        if(volt_vbat > 3800)
        {
            battery_log(BAT_LOG_CRTI, "[swchr_flow_main] wait HW re-turn-on M3, vbat=%d\n", volt_vbat);
        }
        else
        {
            //set ICH
            ...
            if(volt_vbat >= 3400)
            {
                ...
                battery_log(BAT_LOG_CRTI, "[swchr_flow_main] set CV_VTH=3.4V\n");
                ...
                if(reg_val == 1)
                {
                    battery_log(BAT_LOG_CRTI, "[swchr_flow_main] check m3_en==1 => OK\n");
                    //Notmal mode charging
                    swchr_flow_normal(chr_cur_val);
                ...
                {
                    battery_log(BAT_LOG_CRTI, "[swchr_flow_main] check m3_en==0 => FAIL\n");
                }
            }
        }

        battery_log(BAT_LOG_CRTI,"[swchr_flow_main] volt_vbat=%d\n", volt_vbat);
    }

    pmic_read_interface(0x805E, &is_charge_complete, 0x1, 10); //RGS_CHARGE_COMPLETE_DET
    battery_log(BAT_LOG_FULL,"[swchr_flow_main] read RGS_CHARGE_COMPLETE_DET=%d\n", is_charge_complete);

    pmic_read_interface(0x805E, &is_auto_recharge, 0x1, 11); //RGS_AUTO_RECHARGE
    battery_log(BAT_LOG_FULL,"[swchr_flow_main] read RGS_AUTO_RECHARGE=%d\n", is_auto_recharge);
...
static kal_uint32 charging_get_charging_status(void *data)
{
    ...
    battery_log(BAT_LOG_FULL,"[charging_get_charging_status] read RGS_CHARGE_COMPLETE_DET=%d\n", ret_val);
...
{
	kal_uint32 reg_val_1 = 0;
	kal_uint32 reg_val_2 = 0;
	...
	reg_val_1 = upmu_get_reg_value(0x8D1E);
	reg_val_2 = upmu_get_reg_value(0x8D2C);

	if ((is_sw_en == 0 && is_sw_mode == 0 && ((reg_val_1 != 0x2) || (reg_val_2 != 0)))
		|| (is_sw_en == 0 && is_sw_mode == 1 && ((reg_val_1 != 0x2) || (reg_val_2 != 0x30)))
		|| (is_sw_en == 1 && is_sw_mode == 0 && ((reg_val_1 != 0x32) || (reg_val_2 != 0)))
	){
		battery_log(BAT_LOG_CRTI, "[set_usb_dc_in_mode] Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
			0x8D1E, reg_val_1, 0x8D2C, reg_val_2);
	}
}
...
// chr_plug_out_sw_detect
    ...
    if(upmu_is_chr_det()==1) //sync USB device/otg state
    {
        ...
        if(cv_val == 1)
        {
            battery_log(BAT_LOG_FULL,"[chr_plug_out_sw_detect] in CV\n");
            #if 0
        ...
        {
            battery_log(BAT_LOG_FULL,"[chr_plug_out_sw_detect] not in CV\n");
        ...
        #if 1
            battery_log(BAT_LOG_FULL,"[chr_plug_out_sw_detect] Reg[0x%x]=0x%x\n", 0x8074, upmu_get_reg_value(0x8074) );
            battery_log(BAT_LOG_FULL,"[chr_plug_out_sw_detect] ich_low_val=%d, is_charge_complete=%d, is_m3_en=%d\n",
            ...
            if( (ich_low_val==1) || (is_charge_complete==1) )
            {
                ...
                {
                    battery_log(BAT_LOG_CRTI,"[chr_plug_out_sw_detect] nothing 1\n");
                }
            }
            else
            {
                set_usb_dc_in_mode(0,0);
                battery_log(BAT_LOG_CRTI,"[chr_plug_out_sw_detect] nothing 2\n");
            }
#ifdef CONFIG_MTK_ENABLE_PP
            //------------------------------------------------
            ...
            {
                pp_en_count++;
                battery_log(BAT_LOG_CRTI,"[power_path_workaround] disable power_path_workaround (%d,%d)\n", pp_en_count, pp_en_count_go);
            }
#endif
        #endif
        ...
        battery_log(BAT_LOG_FULL,"[chr_plug_out_sw_detect] Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
                0x8D1E, upmu_get_reg_value(0x8D1E), 0x8D2C, upmu_get_reg_value(0x8D2C), 0x816C, upmu_get_reg_value(0x816C) );
    }
    else
    {
        pp_en_count=0;
        battery_log(BAT_LOG_FULL,"[chr_plug_out_sw_detect] no cable\n");
    }
#endif
...
static kal_uint32 charging_get_battery_status(void *data)
{
    ...
    *(kal_bool*)(data) = 0; // battery exist
    battery_log(BAT_LOG_CRTI,"[charging_get_battery_status] no HW function\n");
#else
    kal_uint32 ret=0;
...
static kal_uint32 charging_get_charger_det_status(void *data)
{
    ...
    if (val == 1) // double check
    {
        ...
        if(val_1 < 1000)
        {
            battery_log(BAT_LOG_CRTI,"CHRDET=%d but vchr=%d \n", val, val_1);
            val = 0;
        }
    }

    *(kal_bool*)(data) = val;
	battery_log(BAT_LOG_FULL, "CHRDET=%d\n", val);
    ...
    if(val == 1)
    {
        ...
        battery_log(BAT_LOG_FULL,"[charging_get_charger_det_status] is_valid_charger = %d\n", is_valid_charger);
...
static kal_uint32 charging_set_ta_current_pattern(void *data)
{
    ...
    battery_log(BAT_LOG_CRTI, "[charging_set_ta_current_pattern] [0x%x]=0x%x\n",
        0x8D36, upmu_get_reg_value(0x8D36)
        );

    if(increase == KAL_TRUE)
    {
         battery_log(BAT_LOG_CRTI, "mtk_ta_increase() start (%d)\n", set_ta_off_current_reg_value);
