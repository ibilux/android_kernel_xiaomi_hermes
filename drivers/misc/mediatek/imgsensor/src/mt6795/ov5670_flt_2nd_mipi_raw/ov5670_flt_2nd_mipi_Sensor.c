...
char Module_information[20] = "sunny";
char strmonth[3];
char strday[3];
...
static int RG_Ratio_Typical_Sunny_flt = 0x14B;   // 315
static int BG_Ratio_Typical_Sunny_flt = 0x13C;	//291
...
static int check_otp_wb(int index)
{
	printk("pxs_ov5670_flt_otp ++++++++++++++++  %s \n", __func__); //pei_add	
		...
		if (flag == 0x00) {
		printk("pxs_ov5670_flt_otp  retun 0,group index is empty; 		  %s \n",__func__); //pei_add				
			return 0;
		}
		else if (flag & 0x02) {
		printk("pxs_ov5670_flt_otp  return 1,group index has invalid data; %s \n", __func__); //pei_add		
			return 1;
		}
		else {
		printk("pxs_ov5670_flt_otp  return 2 ,group index has valid data ; %s \n", __func__); //pei_add		
			return 2;
		}
}
...
static kal_uint16 read_otp_module_id(void)
{
	...
	struct otp_struct current_otp;
		...
		printk("pxs_ov5670_flt_otp	otp_index=%d,  %s \n",otp_index, __func__); //pei_add		
		if (i>3) {
		// no valid wb OTP data
		printk("pxs_ov5670_flt_otp	no valid wb OTP data  %s \n", __func__); //pei_add
			return 1;
		}
...
printk("pxs_ov5670_flt_otp AA1=%x ,AA2=%x ,AA3=%x ,AA4=%x ,AA5=%x ,AA6=%x \n ", AA1,AA2,AA3,AA4,AA5,AA6);
...
printk("pxs_ov5670_flt_otp2 AA11=%x ,AA12=%x ,AA13=%x ,AA14=%x ,AA15=%x ,AA16=%x \n ", AA11,AA12,AA13,AA14,AA15,AA16);
...
static int update_otp_wb()
{
	printk("pxs_ov5670_flt_otp ++++++++++++++++  %s \n", __func__); //pei_add	
		...
		printk("pxs_ov5670_flt_otp	otp_index=%d,  otp_info_index=%d,   %s \n",otp_index, otp_info_index, __func__); //pei_add		
		if (i>3) {
		// no valid wb OTP data
		printk("pxs_ov5670_flt_otp	no valid wb OTP data  %s \n", __func__); //pei_add
			return 1;
		...
		if (module_temp == 1)
		{
		strcat(Module_information,strmonth);
		strcat(Module_information,"-");
		strcat(Module_information,strday);
		module_temp--;
		}
		if (IR_Id == 1)
		{
		strcpy(Module_information,"sunny");
		}
	...
	if (IR_Id == 0x02)
	{
		switch(current_otp.module_integrator_id)
		{
			case 0x07:
				...
				break;
			case 0x01:
				RG_Ratio_Typical = RG_Ratio_Typical_Sunny_flt;
				BG_Ratio_Typical = BG_Ratio_Typical_Sunny_flt;
				break;
			default:
				break;
		}
...
printk("pxs_ov5670_flt_otp  R_gain=%x, G_gain=%x, B_gain=%x,  %s \n",R_gain ,B_gain,G_gain, __func__); //pei_add	
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == 0x5670) {				
				...
				if(devinfo_add !=0)
			    {
				    ...
				    dev_sunny->device_ic = "OV5670_flt";
