...
void _i2c_dump_info(mt_i2c *i2c)
{
	...
	I2CLOG("before enable DMA register(0x%ld):\n"
	       ...
	       g_dma_data[i2c->id].tx_4g_support, g_dma_data[i2c->id].rx_4g_support);
	...
	pmic_read_interface(MT6331_SYSLDO_CON3, &val, MT6331_PMIC_RG_VCAM_IO_EN_MASK,
			    MT6331_PMIC_RG_VCAM_IO_EN_SHIFT);
			...
			while (data_size--) {
				*ptr = i2c_readl(i2c, OFFSET_DATA_PORT);
				...
				if (1 == dump_flag) {
					if (0 == *ptr) {
						I2CLOG
						    (" 9210 read 0xd8 fifo: slave(0x%x) fifo=0x%x, pid(%d)\n",
...
static void _i2c_write_reg(mt_i2c *i2c)
{
	...
	if (i2c->dma_en) {
		...
		if (enable_4G()) {
			mt_reg_sync_writel(0x1, i2c->pdmabase + OFFSET_TX_4G_SUPPORT);
			mt_reg_sync_writel(0x1, i2c->pdmabase + OFFSET_RX_4G_SUPPORT);
		}
