...
/*---------------------------------------------------------------------------*/
static struct gyro_hw cust_gyro_hw_lsm = {
    .i2c_num = 3,
...
/*---------------------------------------------------------------------------*/
struct gyro_hw* get_cust_gyro_hw_lsm(void) 
{
    return &cust_gyro_hw_lsm;
}
...
#define LSM6DS3_TO_BOSCH160_SEN   1148       //70 mdps *16.4
...
/*----------------------------------------------------------------------------*/
static int LSM6DS3_ReadGyroData(struct i2c_client *client, char *buf, int bufsize,bool rawdata_mode)
{
	char databuf[6];	
	int data[3];
	struct lsm6ds3_gyro_i2c_data *obj = i2c_get_clientdata(client);  
...
static ssize_t show_sensordata_value(struct device_driver *ddri, char *buf)
{
	...
	LSM6DS3_ReadGyroData(client, strbuf, LSM6DS3_BUFSIZE,false);
...
static ssize_t show_rawdata_value(struct device_driver *ddri, char *buf)
{
	...
	/*if(sensor_power == false)
	{
		LSM6DS3_gyro_SetPowerMode(priv->client, true);
	}*/

	if(hwmsen_read_block(priv->client, LSM6DS3_OUTX_L_G, databuf, 6))
	{
		GYRO_ERR("LSM6DS3 read gyroscope data  error\n");
		return -2;
	}
	...
	{
		priv->data[LSM6DS3_AXIS_X] = (s16)((databuf[LSM6DS3_AXIS_X*2+1] << 8) | (databuf[LSM6DS3_AXIS_X*2]));
		priv->data[LSM6DS3_AXIS_Y] = (s16)((databuf[LSM6DS3_AXIS_Y*2+1] << 8) | (databuf[LSM6DS3_AXIS_Y*2]));
		priv->data[LSM6DS3_AXIS_Z] = (s16)((databuf[LSM6DS3_AXIS_Z*2+1] << 8) | (databuf[LSM6DS3_AXIS_Z*2]));	
 

        offset[LSM6DS3_AXIS_X] = priv->cali_sw[LSM6DS3_AXIS_X] *1000/LSM6DS3_GYRO_SENSITIVITY_2000DPS/131;
        offset[LSM6DS3_AXIS_Y] = priv->cali_sw[LSM6DS3_AXIS_Y] *1000/LSM6DS3_GYRO_SENSITIVITY_2000DPS/131;
        offset[LSM6DS3_AXIS_Z] = priv->cali_sw[LSM6DS3_AXIS_Z] *1000/LSM6DS3_GYRO_SENSITIVITY_2000DPS/131;
	
    	priv->data[LSM6DS3_AXIS_X] +=(s16) offset[LSM6DS3_AXIS_X];
		priv->data[LSM6DS3_AXIS_Y] +=(s16) offset[LSM6DS3_AXIS_Y];
		priv->data[LSM6DS3_AXIS_Z] +=(s16) offset[LSM6DS3_AXIS_Z];
    	/*remap coordinate*/
		data[priv->cvt.map[LSM6DS3_AXIS_X]] = priv->cvt.sign[LSM6DS3_AXIS_X]*priv->data[LSM6DS3_AXIS_X];
		data[priv->cvt.map[LSM6DS3_AXIS_Y]] = priv->cvt.sign[LSM6DS3_AXIS_Y]*priv->data[LSM6DS3_AXIS_Y];
		data[priv->cvt.map[LSM6DS3_AXIS_Z]] = priv->cvt.sign[LSM6DS3_AXIS_Z]*priv->data[LSM6DS3_AXIS_Z];

        // DO DATA TRANSFORMATION from ST TO BOSCH
		data[priv->cvt.map[LSM6DS3_AXIS_X]] = data[priv->cvt.map[LSM6DS3_AXIS_X]]*LSM6DS3_TO_BOSCH160_SEN/1000;
		data[priv->cvt.map[LSM6DS3_AXIS_Y]] = data[priv->cvt.map[LSM6DS3_AXIS_Y]]*LSM6DS3_TO_BOSCH160_SEN/1000;
		data[priv->cvt.map[LSM6DS3_AXIS_Z]] = data[priv->cvt.map[LSM6DS3_AXIS_Z]]*LSM6DS3_TO_BOSCH160_SEN/1000;

    //GYRO_ERR("u###s xyz %hd,%hd,%hd \n",data[LSM6DS3_AXIS_X], data[LSM6DS3_AXIS_Y], data[LSM6DS3_AXIS_Z]); 
		...
		const char *buf, size_t count)
{
	...
	err = kstrtoul(buf, 10, &power_mode);
        ...
        GYRO_ERR(" get powermode is %ld \n",power_mode);

	if (err == 0) {
		  if (power_mode == 0)
                          err = LSM6DS3_gyro_SetPowerMode( priv->client, false);	

		  if (power_mode ==1)
                          err = LSM6DS3_gyro_SetPowerMode( priv->client, true);	
	      return count;
	}else{
               GYRO_ERR("store_power_mode_value get value fail \n");
          return err;
		}
		...
		const char *buf, size_t count)
{
	...
	err = kstrtoul(buf, 10, &range);
        GYRO_ERR(" get store_range_value is %ld \n",range);
		
	if (err == 0) {
		if (range == BMG_RANGE_2000)
			 err = LSM6DS3_gyro_SetFullScale(priv->client,LSM6DS3_GYRO_RANGE_2000DPS); 
		if (range == BMG_RANGE_1000)
			 err = LSM6DS3_gyro_SetFullScale(priv->client,LSM6DS3_GYRO_RANGE_1000DPS); 
		if (range == BMG_RANGE_500)
			  err = LSM6DS3_gyro_SetFullScale(priv->client,LSM6DS3_GYRO_RANGE_500DPS); 
		if (range == BMG_RANGE_250)
			  err = LSM6DS3_gyro_SetFullScale(priv->client,LSM6DS3_GYRO_RANGE_245DPS); 
		if (err)
			return err;
		return count;
		}
		...
		const char *buf, size_t count)
{
	...
	err = kstrtoul(buf, 10, &datarate);
        GYRO_ERR(" get store_datarate_value is %ld \n",datarate);
		
	if (err == 0) {
                if(datarate ==8)
			err= LSM6DS3_gyro_SetSampleRate(priv->client, LSM6DS3_ACC_ODR_104HZ);
	        if(datarate >=9)
			err= LSM6DS3_gyro_SetSampleRate(priv->client, LSM6DS3_ACC_ODR_416HZ);	
		if (err)
			return err;
		return count;
	}
...
static DRIVER_ATTR(powermode, S_IWUSR ,
		NULL, store_power_mode_value);
static DRIVER_ATTR(range, S_IWUSR ,
		NULL, store_range_value);
static DRIVER_ATTR(datarate, S_IWUSR ,
		NULL, store_datarate_value);
...
static int lsm6ds3_gyro_get_data(int* x ,int* y,int* z, int* status)
{
	...
	LSM6DS3_ReadGyroData(priv->client, buff, LSM6DS3_BUFSIZE,false);
...
{
	struct gyro_hw *gy_hw = get_cust_gyro_hw_lsm();
	GYRO_FUN();
...
{
	//GYRO_FUN();
	struct gyro_hw *hw = get_cust_gyro_hw_lsm();
