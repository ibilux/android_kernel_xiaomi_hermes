...
u32 msdc_dat1_ctrl_reg_offset[4] = {
	MSDC0_GPIO_DAT_OFFSET, MSDC1_GPIO_DAT1_OFFSET, MSDC2_GPIO_DAT1_OFFSET,
	MSDC3_GPIO_DAT1_OFFSET };

void msdc_dump_padctl(struct msdc_host *host, u32 pin)
{
	switch (pin) {
	case GPIO_CLK_CTRL:
		INIT_MSG("  CLK_PAD_CTRL[0x%p]        = 0x%.8x",
			 MSDC_GPIO_ADDR(msdc_clk_ctrl_reg_offset[host->id]),
			 sdr_read32(MSDC_GPIO_ADDR(msdc_clk_ctrl_reg_offset[host->id])));
		break;
	case GPIO_CMD_CTRL:
		INIT_MSG("  CMD_PAD_CTRL[0x%p]        = 0x%.8x",
			 MSDC_GPIO_ADDR(msdc_cmd_ctrl_reg_offset[host->id]),
	...
	case GPIO_RST_CTRL:
		if (host->id == 0) {
			INIT_MSG("  RST_PAD_CTRL[0x%p]        = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC0_GPIO_RST_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC0_GPIO_RST_OFFSET)));
		}
		break;
	case GPIO_DS_CTRL:
		if (host->id == 0) {
			INIT_MSG("  DS_PAD_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC0_GPIO_DS_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC0_GPIO_DS_OFFSET)));
		}
		break;
	case GPIO_PAD_CTRL:
		INIT_MSG("  TDRD_PAD_CTRL[0x%p]           = 0x%.8x",
			 MSDC_GPIO_ADDR(msdc_pad_ctrl_reg_offset[host->id]),
			 sdr_read32(MSDC_GPIO_ADDR(msdc_pad_ctrl_reg_offset[host->id])));
		break;
	case GPIO_MODE_CTRL:
		if (host->id == 0) {
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC0_GPIO_MODE0_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC0_GPIO_MODE0_OFFSET)));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC0_GPIO_MODE1_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC0_GPIO_MODE1_OFFSET)));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC0_GPIO_MODE2_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC0_GPIO_MODE2_OFFSET)));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC0_GPIO_MODE3_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC0_GPIO_MODE3_OFFSET)));
		} else if (host->id == 1) {
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC1_GPIO_MODE0_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC1_GPIO_MODE0_OFFSET)));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC1_GPIO_MODE1_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC1_GPIO_MODE1_OFFSET)));
		} else if (host->id == 2) {
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC2_GPIO_MODE0_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC2_GPIO_MODE0_OFFSET)));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC2_GPIO_MODE1_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC2_GPIO_MODE1_OFFSET)));
		} else if (host->id == 3) {
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC3_GPIO_MODE0_OFFSET),
				 sdr_read32(MSDC_GPIO_ADDR(MSDC3_GPIO_MODE0_OFFSET)));
			INIT_MSG("  PAD_MODE_CTRL[0x%p]         = 0x%.8x",
				 MSDC_GPIO_ADDR(MSDC3_GPIO_MODE1_OFFSET),
...
#ifdef CONFIG_OF
	INIT_MSG(" pll[0x1020925c][bit0~1 should be 0x1]=0x%x",
		 sdr_read32(msdc_apmixed_base + 0x25c));
	INIT_MSG(" pll[0x10209250][bit0 should be 0x1]=0x%x",
		 sdr_read32(msdc_apmixed_base + 0x250));
	INIT_MSG(" mux[0x10000070][should be 0x02060301]=0x%x",
		 sdr_read32(msdc_topckgen_base + 0x070));
	INIT_MSG(" clk[0x10003018][bit13=msdc0, bit14=msdc1]=0x%x",
		 sdr_read32(msdc_topckgen_base + 0x018));
#else
	...
	for (i = 0; i < 5; i++) {
		for (addr = 0; addr < 0x78; addr += 4) {
			pr_err("reg[0x%x]=0x%x", (0x10203500 + addr),
			       sdr_read32((msdc_emi_base + 0x500 + addr)));
			if (addr % 0x10 == 0)
				mdelay(1);
		}
	}
	...
	while (sdr_read32(msdc_pericfg_base + 0x214) & 0xc) {
		if (++i < 300) {
			...
			pr_err("[%s]: check peri-bus: 0x%x at %d\n", __func__,
			       sdr_read32(msdc_pericfg_base + 0x214), line);
...
static void msdc_clr_fifo(unsigned int id)
{
	...
	if (sdr_read32(MSDC_DMA_CFG) & MSDC_DMA_CFG_STS) {
		...
		if (retry == 0) {
			pr_err
			    ("<<<WARN>>>: msdc%d, faield to stop DMA before clear FIFO, MSDC_DMA_CFG=0x%x\n",
...
int top_reset_restore_msdc_cfg(int restore)
{
	...
	if (restore) {
		msdc_retry(!(sdr_read32(mtk_msdc_host[0]->base + OFFSET_MSDC_CFG) & MSDC_CFG_CKSTB),
			   retry, count, 0);
		if ((retry == 0) && (count == 0)) {
			return 1;
		}
...
void msdc_set_rdsel(struct msdc_host *host, bool sd_18)
{
	...
	case 1:
		if (sd_18) {
			sdr_set_field(MSDC_GPIO_ADDR(MSDC1_GPIO_PAD_OFFSET), GPIO_PAD_RDSEL_MASK,
				      0x0);
		} else {
			sdr_set_field(MSDC_GPIO_ADDR(MSDC1_GPIO_PAD_OFFSET), GPIO_PAD_RDSEL_MASK,
				      0xc);
	...
	case 2:
		if (sd_18) {
			sdr_set_field(MSDC_GPIO_ADDR(MSDC2_GPIO_PAD_OFFSET), GPIO_PAD_RDSEL_MASK,
				      0x0);
		} else {
			sdr_set_field(MSDC_GPIO_ADDR(MSDC2_GPIO_PAD_OFFSET), GPIO_PAD_RDSEL_MASK,
				      0xc);
...
void msdc_set_tdsel(struct msdc_host *host, bool sleep)
{
	...
	case 1:
		if (sleep) {
			sdr_set_field(MSDC_GPIO_ADDR(MSDC1_GPIO_PAD_OFFSET), GPIO_PAD_TDSEL_MASK,
				      0xf);
		} else {
			sdr_set_field(MSDC_GPIO_ADDR(MSDC1_GPIO_PAD_OFFSET), GPIO_PAD_TDSEL_MASK,
				      0xa);
	...
	case 2:
		if (sleep) {
			sdr_set_field(MSDC_GPIO_ADDR(MSDC2_GPIO_PAD_OFFSET), GPIO_PAD_TDSEL_MASK,
				      0xf);
		} else {
			sdr_set_field(MSDC_GPIO_ADDR(MSDC2_GPIO_PAD_OFFSET), GPIO_PAD_TDSEL_MASK,
				      0xa);
...
static void msdc_pin_pud(struct msdc_host *host, u32 pin, u32 mode, u32 val)
{
	...
	case GPIO_CLK_CTRL:
		sdr_set_field(MSDC_GPIO_ADDR(msdc_clk_ctrl_reg_offset[host->id]),
			      GPIO_MSDC_R1R0_MASK, val);
		sdr_set_field(MSDC_GPIO_ADDR(msdc_clk_ctrl_reg_offset[host->id]),
			      GPIO_MSDC_PUPD_MASK, mode);
		break;
	case GPIO_CMD_CTRL:
	...
	case GPIO_DAT_CTRL:
		if (0 == host->id) {
			...
			sdr_set_field(MSDC_GPIO_ADDR(msdc_dat1_ctrl_reg_offset[host->id]),
				      GPIO_MSDC_DAT0_R1R0_MASK, val);
			sdr_set_field(MSDC_GPIO_ADDR(msdc_dat1_ctrl_reg_offset[host->id]),
				      GPIO_MSDC_DAT0_PUPD_MASK, mode);

			sdr_set_field(MSDC_GPIO_ADDR(msdc_dat1_ctrl_reg_offset[host->id]),
				      GPIO_MSDC_DAT1_R1R0_MASK, val);
			sdr_set_field(MSDC_GPIO_ADDR(msdc_dat1_ctrl_reg_offset[host->id]),
				      GPIO_MSDC_DAT1_PUPD_MASK, mode);

			sdr_set_field(MSDC_GPIO_ADDR(msdc_dat1_ctrl_reg_offset[host->id]),
				      GPIO_MSDC_DAT2_R1R0_MASK, val);
			sdr_set_field(MSDC_GPIO_ADDR(msdc_dat1_ctrl_reg_offset[host->id]),
				      GPIO_MSDC_DAT2_PUPD_MASK, mode);

			sdr_set_field(MSDC_GPIO_ADDR(msdc_dat1_ctrl_reg_offset[host->id]),
				      GPIO_MSDC_DAT3_R1R0_MASK, val);
			sdr_set_field(MSDC_GPIO_ADDR(msdc_dat1_ctrl_reg_offset[host->id]),
				      GPIO_MSDC_DAT3_PUPD_MASK, mode);
		}
		break;
	case GPIO_RST_CTRL:
		if (0 == host->id) {
			sdr_set_field(MSDC_GPIO_ADDR(MSDC0_GPIO_RST_OFFSET), GPIO_MSDC_R1R0_MASK,
				      val);
			sdr_set_field(MSDC_GPIO_ADDR(MSDC0_GPIO_RST_OFFSET), GPIO_MSDC_PUPD_MASK,
				      mode);
		}
		break;
	case GPIO_DS_CTRL:
		if (0 == host->id) {
			sdr_set_field(MSDC_GPIO_ADDR(MSDC0_GPIO_DS_OFFSET), GPIO_MSDC_R1R0_MASK,
				      val);
			sdr_set_field(MSDC_GPIO_ADDR(MSDC0_GPIO_DS_OFFSET), GPIO_MSDC_PUPD_MASK,
				      mode);
		}
		break;
	default:
...
void msdc_set_smt(struct msdc_host *host, u32 pin, u32 val)
{
	...
	case GPIO_RST_CTRL:
		if (0 == host->id)
			sdr_set_field(MSDC_GPIO_ADDR(MSDC0_GPIO_RST_OFFSET), GPIO_MSDC_SMT_MASK,
				      val);
		break;
	case GPIO_DS_CTRL:
		if (0 == host->id)
			sdr_set_field(MSDC_GPIO_ADDR(MSDC0_GPIO_DS_OFFSET), GPIO_MSDC_SMT_MASK,
				      val);
		break;
	default:
		break;
...
static void msdc_emmc_power(struct msdc_host *host, u32 on)
{
	...
	if (host->mmc && host->mmc->card && (host->mmc->caps & MMC_CAP_WAIT_WHILE_BUSY)
		...
		if ((DIV_ROUND_UP(host->mmc->card->ext_csd.sa_timeout, 10000000 / HZ)) < HZ)
			tmo =
			    jiffies + DIV_ROUND_UP(host->mmc->card->ext_csd.sa_timeout,
						   10000000 / HZ) + HZ / 100;
		else {
			tmo = jiffies + HZ;
		}
...
static void msdc_set_bad_card_and_remove(struct msdc_host *host)
{
	...
	if (host->mmc->card) {
		...
		ERR_MSG("Do remove the bad card, block_bad_card=%d, card_inserted=%d",
...
static void msdc_set_timeout(struct msdc_host *host, u32 ns, u32 clks)
{
	...
	N_MSG(OPS,
	      "msdc%d, Set read data timeout: %dns %dclks -> %d x 1048576  cycles, mode:%d, clk_freq=%dKHz\n",
...
static void msdc_tasklet_card(unsigned long arg)
{
	...
	if (hw->flags & MSDC_UHS1) {
		host->mmc->caps |= MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 | MMC_CAP_UHS_SDR50
			| MMC_CAP_UHS_SDR104;	/* | MMC_CAP_SET_XPC_180; */
	...
	if (mmc_card_sd(card) && (host->hw->host_function == MSDC_SD)) {
		if ((host->power_cycle < MSDC_MAX_POWER_CYCLE) && (host->power_cycle_enable)) {
			...
			if (host->power_control)
				...
				ERR_MSG
				    ("No power control callback. Please check host_function<0x%lx>",
...
static void msdc_pm(pm_message_t state, void *data)
{
	...
	if ((evt == PM_EVENT_SUSPEND) || (evt == PM_EVENT_USER_SUSPEND)) {
		if ((host->hw->host_function == MSDC_SDIO) && (evt == PM_EVENT_USER_SUSPEND))
			pr_err("msdc%d -> MSDC Device Request Suspend", host->id);
				       ...
				       unsigned long timeout)
{
	...
	/* if the block number is bigger than 0xFFFF, then CMD23 arg will be failed to set it. */
				if (sdr_read32(SDC_BLK_NUM) != (sbc->arg & 0xFFFF))
					pr_err
	("msdc%d: acmd23 arg(0x%x) failed to match the read block num(0x%x), SDC_BLK_NUM(0x%x)\n",
...
/* if the block number is bigger than 0xFFFF, then CMD23 arg will be failed to set it. */
				if (sdr_read32(SDC_BLK_NUM) != (sbc->arg & 0xFFFF))
					pr_err
	("msdc%d: acmd23 arg(0x%x) failed to match the write block num(0x%x), SDC_BLK_NUM(0x%x)\n",
					     host->id, sbc->arg, host->mrq->cmd->data->blocks,
					     sdr_read32(SDC_BLK_NUM));
				else
					sdr_write32(SDC_BLK_NUM, sbc->arg);
					      ...
					      int tune, unsigned long timeout)
{
	...
	if (g_err_tune_dbg_error && (g_err_tune_dbg_count > 0) && (g_err_tune_dbg_host == host->id)) {
		...
		if ((g_err_tune_dbg_cmd == MMC_SET_BLOCK_COUNT) &&
			...
			if (g_err_tune_dbg_error & MTK_MSDC_ERROR_ACMD_TMO) {
				...
				g_err_tune_dbg_count--;
				pr_err
				    ("[%s]: got the error cmd:%d, dbg error=%d, sbc->error=%d, count=%d\n",
				     __func__, g_err_tune_dbg_cmd, g_err_tune_dbg_error, sbc->error,
				     g_err_tune_dbg_count);
	...
	if (intsts & cmdsts) {
		...
		if ((intsts & MSDC_INT_CMDRDY) || (intsts & MSDC_INT_ACMDRDY) ||
			...
			default:	/* Response types 1, 3, 4, 5, 6, 7(1b) */
				*rsp = sdr_read32(SDC_RESP0);
				...
				if (((cmd->opcode == 13) || (cmd->opcode == 25))
				    ...
				    && (host->hw->host_function == MSDC_EMMC)) {
					pr_err
					    ("[%s]: msdc%d XXX CMD<%d> resp<0x%.8x>,bit31=1,force make crc error\n",
			...
			/* msdc_dump_info(host->id); */
			if ((host->hw->host_function != MSDC_SDIO)
			    && ((MMC_RSP_R1B == mmc_resp_type(cmd))
				|| (cmd->opcode == MMC_SEND_STATUS))) {
				pr_err
				    ("[%s]: msdc%d XXX CMD<%d> ARG<0x%.8X> is R1B/CMD13, CRC not reset hw...\n",
				     __func__, host->id, cmd->opcode, cmd->arg);
			} else {
				msdc_reset_hw(host->id);
			}
...
#ifdef MTK_SDIO30_ONLINE_TUNING_SUPPORT
	if ((host->hw->host_function != MSDC_SDIO) || (atomic_read(&host->ot_work.autok_done) != 0)
		|| (host->mclk <= 50 * 1000 * 1000))	/* auto-K have done or finished */
								{
									msdc_dump_info(host->id);
								}
#else
...
#ifdef MTK_SDIO30_ONLINE_TUNING_SUPPORT
					if ((host->hw->host_function != MSDC_SDIO)
|| (atomic_read(&host->ot_work.autok_done) != 0) || (host->mclk <= 50 * 1000 * 1000))
...
static void msdc_dma_stop(struct msdc_host *host)
{
	...
	if (retry == 0) {
		...
		mdelay(10);
		ERR_MSG
		    ("######################## Failed to stop DMA! finish dump#############################");
...
static int ot_do_command(struct msdc_host *host, u32 rawcmd, u32 rawarg, u32 *pintsts)
{
	...
	if (ot_command_resp_polling(host, cmd, 0, CMD_TIMEOUT, pintsts)) {
		pr_err("[SDIO_OnlineTuning]ot_command_resp_polling fail, err = %d, resp = 0x%.8x\n",
		      ...
		      u32 CRCPass, bool rw)
{
	...
	if (ot_do_command(host, rawcmd, rawarg, &intsts) != 0) {
		...
		if (ret == (unsigned int)-ETIMEDOUT) {
			...
			ret1 = ot_recover_device(host, potData->fn, potData->addr);
/* if ot_recover_device returns 0, means device recover success, continue online tuning */
			if (ret1 == 0) {
	...
	sdr_set_field(MSDC_PAD_TUNE, MSDC_PAD_TUNE_CMDRDLY, potData->cmdrdly);	/* Set cmd delay */
		...
		("[SDIO_OnlineTuning](TMO) host->cmd->resp[3] = 0x%x, next dat3 delay = %d\n",
				...
				if (reset) {
					if (potData->orig_ddlsel == 0) {
						u32 crcsts =
						    (host->cmd->resp[0] & host->
						     cmd->resp[1] & host->cmd->resp[2] & host->
						     cmd->resp[3]);
						u32 datrddly = ot_find_next_gear(crcsts,
										 potData->
										 tune_wind_size,
										 potData->dat0rddly,
										 &potData->
					...
					if (potData->eco_ver >= 4) {
						rxdly0 =
						    (potData->
						     dat0rddly << 24) | (potData->dat1rddly << 16) |
						    (potData->
						     dat2rddly << 8) | (potData->dat3rddly << 0);
					} else {
						rxdly0 =
						    (potData->
						     dat0rddly << 0) | (potData->dat1rddly << 8) |
						    (potData->
						     dat2rddly << 16) | (potData->dat3rddly << 24);
					}
	...
	if (potData->orig_ddlsel == 0) {
		u32 crcsts =
		    (host->cmd->resp[0] & host->cmd->resp[1] & host->cmd->resp[2] & host->
		     cmd->resp[3]);
		if (crcsts != CRCPass) {
			u32 datrddly =
		...
		if (potData->eco_ver >= 4) {
			rxdly0 =
			    (potData->dat0rddly << 24) | (potData->
							  dat1rddly << 16) | (potData->dat2rddly <<
									      8) | (potData->
										    dat3rddly << 0);
		} else {
			rxdly0 =
			    (potData->dat0rddly << 0) | (potData->
							 dat1rddly << 8) | (potData->dat2rddly <<
									    16) | (potData->
										   dat3rddly << 24);
		}
		sdr_write32(MSDC_DAT_RDDLY0, rxdly0);
...
_retry:
	...
	if (otData.cmddlypass == 0) {
		...
		ret =
		    ot_adjust_tunewnd(host, &otData.fCmdTestedGear, &tune_wind_size_cmd, &CRCPass);
...
out:
	...
	if (ret < 0) {
		...
		if (otData.eco_ver >= 4) {
			rxdly0 = (otData.orig_dat0rddly << 24) | (otData.orig_dat1rddly << 16)
			    | (otData.orig_dat2rddly << 8) | (otData.orig_dat3rddly << 0);
		} else {
			rxdly0 = (otData.orig_dat0rddly << 0) | (otData.orig_dat1rddly << 8)
...
/* need ask the designer, how about autocmd12 or autocmd23 with pio mode */
			host->autocmd &= ~MSDC_AUTOCMD12;
...
static int msdc_lower_freq(struct msdc_host *host)
{
	...
	if (div >= MSDC_MAX_FREQ_DIV) {
		...
		msdc_clk_stable(host, mode, div, hs400_src);
		host->sclk = (div == 0) ?
		    hclks[host->hw->clk_src] / 2 : hclks[host->hw->clk_src] / (4 * div);
...
int emmc_hs400_tune_rw(struct msdc_host *host)
{
	...
	if (++host->t_counter.time_hs400 == (g_ett_tune ? (32 * 32) : MAX_HS400_TUNE_COUNT)) {
		ERR_MSG
		    ("Failed to update EMMC50_PAD_DS_TUNE_DLY, cur_ds_dly3=0x%x, cur_ds_dly1=0x%x",
...
static void msdc_dump_trans_error(struct msdc_host *host,
				  struct mmc_command *cmd,
				  struct mmc_data *data,
				  struct mmc_command *stop, struct mmc_command *sbc)
{
	/* void __iomem *base = host->base; */
	...
	if (stop) {
		if (host->suspend == 1) {
			ERR_MSG("XXX STOP<%d><0x%x> Error<%d> Resp<0x%x>", stop->opcode, stop->arg,
				stop->error, stop->resp[0]);
		} else {
			INFO_MSG("XXX STOP<%d><0x%x> Error<%d> Resp<0x%x>", stop->opcode, stop->arg,
				 stop->error, stop->resp[0]);
		}
	}
	...
	if (sbc) {
		if (host->suspend == 1) {
			ERR_MSG("XXX SBC<%d><0x%x> Error<%d> Resp<0x%x>", sbc->opcode, sbc->arg,
				sbc->error, sbc->resp[0]);
		} else {
			INFO_MSG("XXX SBC<%d><0x%x> Error<%d> Resp<0x%x>", sbc->opcode, sbc->arg,
				 sbc->error, sbc->resp[0]);
		}
	}
#ifdef MTK_SDIO30_ONLINE_TUNING_SUPPORT
	...
	for (i = 0; i < host->hw->ett_count; i++) {
		...
		if (mode == ett->speed_mode) {
			sdr_set_field((volatile u32 *)(base + ett->reg_addr), ett->reg_offset,
				      ett->value);
			pr_err
			    ("[%s]: msdc%d, reg[0x%x], offset[0x%x], value[0x%x], readback[0x%x]\n",
		...
		if (1 == g_emmc_mode_switch) {
			if (!(mmc->caps2 & MMC_CAP2_HS200_1_8V_SDR)
				...
				if (mmc->card && mmc->card->ext_csd.hs_max_dtr > 52000000) {
					mmc->card->state &=
					    ~(MMC_STATE_HIGHSPEED_200 | MMC_STATE_HIGHSPEED_400);
					if ((mmc->card->ext_csd.
					     raw_card_type & EXT_CSD_CARD_TYPE_MASK) &
		...
		if (ios->clock >= 25000000) {
			...
			if ((host->hw->host_function == MSDC_EMMC)
				...
				sdr_set_field(MSDC_PATCH_BIT1,
					      MSDC_PB1_BUSY_MARGIN,
					      host->saved_para.write_busy_margin);
				sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_CRC_MARGIN,
					      host->saved_para.write_crc_margin);
			}
...
#if defined(FEATURE_MET_MMC_INDEX)
extern void met_mmc_dma_stop(struct mmc_host *host, u32 lba, unsigned int len, u32 opcode,
			     unsigned int bd_num);
#endif
...
static irqreturn_t msdc_irq(int irq, void *dev_id)
{
	...
	if (data != NULL) {
		...
		if (g_err_tune_dbg_error && (g_err_tune_dbg_count > 0)
			...
			if (g_err_tune_dbg_cmd == (sdr_read32(SDC_CMD) & 0x3f)) {
				...
				pr_err
				    ("[%s]: got the error cmd:%d, dbg error 0x%x, data->error=%d, count=%d\n",
				     __func__, g_err_tune_dbg_cmd, g_err_tune_dbg_error,
				     data->error, g_err_tune_dbg_count);
			}
			...
			if (intsts & MSDC_INT_XFER_COMPL) {
				...
				if ((data->mrq != NULL) && (data->mrq->cmd != NULL)) {
					met_mmc_dma_stop(host->mmc, data->mrq->cmd->arg,
							 data->blocks, data->mrq->cmd->opcode,
							 met_mmc_bdnum);
				}
#endif
			}
	...
	if ((cmd != NULL) && (intsts & cmdsts)) {
		...
		if (intsts & MSDC_INT_CMDRDY) {
			...
			cmd->error = (unsigned int)-EIO;
			ERR_MSG("XXX CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%.8x>", cmd->opcode,
				cmd->arg);
			msdc_reset_hw(host->id);
		} else if (intsts & MSDC_INT_CMDTMO) {
			cmd->error = (unsigned int)-ETIMEDOUT;
	...
	if (enable) {
		if ((host->id == 1) && (MSDC_SD == hw->host_function)
			...
			if (0 == host->sd_cd_polarity) {
				...
				mt_eint_registration(CUST_EINT_MSDC1_INS_NUM,
						     CUST_EINTF_TRIGGER_HIGH, msdc1_eint_handler,
						     1);
			}
...
void msdc_dump_gpd_bd(int id)
{
	...
	if (gpd == NULL) {
		...
		pr_err("hwo:0x%x, bdp:0x%x, rsv0:0x%x, chksum:0x%x,intr:0x%x,rsv1:0x%x,next:0x%x\n",
		       ...
		       (unsigned int)gpd->next);
		pr_err
		    ("ptr:0x%x, buflen:0x%x, extlen:0x%x, rsv2:0x%x, arg:0x%x,blknum:0x%x,cmd:0x%x\n",
		     (unsigned int)gpd->ptr, gpd->buflen, gpd->extlen, gpd->rsv2, gpd->arg,
		     gpd->blknum, gpd->cmd);
	}
	pr_err("==========================MSDC BD INFO ===========================\n");
	if (bd == NULL) {
		...
		for (i = 0; i < host->dma.sglen; i++) {
			pr_err("the %d BD\n", i);
			pr_err
			    ("	eol:0x%x, rsv0:0x%x, chksum:0x%x, rsv1:0x%x,blkpad:0x%x,dwpad:0x%x,\n",
			     bd->eol, bd->rsv0, bd->chksum, bd->rsv1, bd->blkpad, bd->dwpad);
			pr_err("	rsv2:0x%x, next:0x%x, ptr:0x%x, buflen:0x%x, rsv3:0x%x\n",
			       bd->rsv2, (unsigned int)bd->next, (unsigned int)bd->ptr,
			       bd->buflen, bd->rsv3);
		}
	}
}
		...
		if (is_card_sdio(mtk_msdc_host[hostidx])) {
			...
			while (1) {
				if (atomic_read(&host->ot_done)) {
					printk
					    ("[%s] msdc%d prev. online tuning done, host->ot_work.chg_volt = %d\n",
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	host->dma.gpd =
	    dma_alloc_coherent(&pdev->dev, MAX_GPD_NUM * sizeof(gpd_t), &host->dma.gpd_addr,
			       GFP_KERNEL);
	host->dma.bd =
	    dma_alloc_coherent(&pdev->dev, MAX_BD_NUM * sizeof(bd_t), &host->dma.bd_addr,
			       GFP_KERNEL);
	...
	pr_err
	    ("[%s]: msdc%d, pll[0x1020925c][bit0~1=0x1]=0x%x, pll[0x10209250][bit0=0x1]=0x%x, mux[0x10000070][bit15,23,31=0x1]=0x%x, clk[0x10003018][bit13=msdc0, bit14=msdc1]=0x%x\n",
	     __func__, host->id, sdr_read32(msdc_apmixed_base + 0x25c),
	     sdr_read32(msdc_apmixed_base + 0x250), sdr_read32(msdc_topckgen_base + 0x070),
