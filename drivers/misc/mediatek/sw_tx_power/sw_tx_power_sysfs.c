...
static ssize_t swtp_attr_show(struct kobject *kobj, struct attribute *attr, char *buf)
{
...
static const struct sysfs_ops swtp_sysfs_ops = {
	.show = swtp_attr_show,
...
static ssize_t swtp_sysfs_show(struct swtp_sysfs_obj *swtp_sysfs_obj,
			       struct swtp_sysfs_attribute *attr, char *buf)
{
	...
	buf_len += sprintf(buf + buf_len, "[SM] : 0x%x, 0x%x, 0x%x\n", swtp_super_state.enable,
			   swtp_super_state.mode, swtp_super_state.setvalue);

	buf_len += sprintf(buf + buf_len, "[UM] : 0x%x, 0x%x, 0x%x\n", swtp_normal_state.enable,
			   swtp_normal_state.mode, swtp_normal_state.setvalue);
	return buf_len;
}
...
static ssize_t swtp_sysfs_store(struct swtp_sysfs_obj *swtp_sysfs_obj,
				struct swtp_sysfs_attribute *attr, const char *buf, size_t len)
{
	char *stop;
	long set_mode, temp;
	int pstrlen;
	...
	if (strstr(buf, "off"))
		...
		pr_err("[swtp] sysfs_store %ld %ld\n", set_mode, temp);
	} else {
		pr_err("[swtp] command error\n");
	}
...
static int __init swtp_sysfs_init(void)
{
	...
	if (kobject_init_and_add(&obj->kobj, &swtp_ktype, NULL, "ctrl")) {
		...
		pr_err("[swtp] kobject_init_and_add.\n");
		return -ENOMEM;
	}
