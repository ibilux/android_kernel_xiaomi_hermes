...
struct timespec g_bat_time_pm_last_trigger;
...
static void battery_timer_resume(void)
{
	...
	battery_log(BAT_LOG_CRTI,
		    "[battery_timer_resume] is_pcm_timer_trigger %d pm_diff_time %ld bat_spm_timeout %d\n",
		    is_pcm_timer_trigger, xts.tv_sec - g_bat_time_pm_last_trigger.tv_sec,
		    bat_spm_timeout);

	if (((is_pcm_timer_trigger == KAL_TRUE)
	     && (xts.tv_sec - g_bat_time_pm_last_trigger.tv_sec >= 5)) || bat_spm_timeout) {
		mutex_lock(&bat_mutex);
		bat_spm_timeout = FALSE;
		sleep_total_time = 0;
		memcpy(&g_bat_time_pm_last_trigger, &xts, sizeof(struct timespec));
		BAT_thread();
		mutex_unlock(&bat_mutex);
	} else {
